/*
 *  $Id: unpack.c,v 1.7 1994/09/21 15:27:18 gropp Exp $
 *
...
#ifndef lint
static char vcid[] = "$Id: unpack.c,v 1.7 1994/09/21 15:27:18 gropp Exp $";
#endif /* lint */
...
MPI_Comm      comm;
{
  ...
  (*position) += pad;
  errno = MPIR_Unpack( outbuf, outcount, type, (char *)inbuf + (*position));
  if (errno) MPIR_ERROR(comm,errno,"Error in MPI_UNPACK" );
  (*position) += size;
  return (errno);
}
...
void *in;
{
  ...
  /* Contiguous types */
  case MPIR_CONTIG:
	errno = MPIR_Unpack ( buf, count * type->count, type->old_type, in );
	break;
  ...
  case MPIR_HVECTOR:
	...
	for (i=0; i<count; i++) {
	  ...
	  for (j=0; j<type->count; j++) {
		if (errno = MPIR_Unpack (lbuf, type->blocklen, 
					 type->old_type, lin)) return errno;
		lbuf  += (type->stride);
		if ((j+1) != type->count)
  ...
  case MPIR_HINDEXED:
	...
	for (i=0; i<count; i++) {
	  for (j=0;j<type->count; j++) {
		...
		if (errno = MPIR_Unpack (tmp_buf, type->blocklens[j], 
					 type->old_type, lin)) return errno;
		if ((j+1) != type->count)
  ...
  case MPIR_STRUCT:
	...
	for (i=0; i<count; i++) {
	  for (j=0;j<type->count; j++) {
		tmp_buf  = lbuf + type->indices[j];
		if (errno = MPIR_Unpack(tmp_buf,type->blocklens[j],
					type->old_types[j],lin)) return errno;
		if ((j+1) != type->count)
