...
#ifdef MPID_DEBUG_ALL   /* #DEBUG_START# */
    if (DebugFlag) {
	MPID_CMMD_Print_pkt_data( "S Getting data from mpid->start", 
			        pkt.buffer, len );
	}
#endif                  /* #DEBUG_END# */
    }
...
#endif
CMMD_send_noblock(dest,MPID_PT2PT_TAG,(char*)(&pkt),len + sizeof(MPID_PKT_HEAD_T));
...
/* Long message */
int MPID_CMMD_post_send_long( dmpi_send_handle, mpid_send_handle, len ) 
MPIR_SHANDLE *dmpi_send_handle;
MPID_SHANDLE *mpid_send_handle;
int len;
{
...
if (mpid_send_handle->is_non_blocking) 
    while (MPID_CMMD_check_incoming( MPID_NOTBLOCKING ) != -1) ;
#endif
CMMD_send_noblock(dest,MPID_PT2PT_TAG,(char*)(&pkt),sizeof(MPID_PKT_LONG_T));
...
#ifndef PI_NO_NSEND
if (mpid_send_handle->is_non_blocking) {
    mpid_send_handle->sid =CMMD_send_async(dest,MPID_PT2PT2_TAG(__MYPROCID),(char*)(address),len,(void*(*)())0,(void*)0);
    }
else 
#endif
    {
    mpid_send_handle->sid = 0;
    CMMD_send_noblock(dest,MPID_PT2PT2_TAG(__MYPROCID),(char*)(address),len);
    DMPI_mark_send_completed( dmpi_send_handle );
    }
#endif
return MPI_SUCCESS;
...
pkt.tag		 = dmpi_send_handle->tag;
pkt.len          = len;
pkt.sync_id      = MPID_CMMD_Get_Sync_Id( dmpi_send_handle, mpid_send_handle );
...
/* In case the message is marked as non-blocking, indicate that we don't
   need to wait on it */
mpid_send_handle->sid = 0;
CMMD_send_noblock(dest,MPID_PT2PT_TAG,(char*)(&pkt),len + (sizeof(MPID_PKT_SHORT_SYNC_T)-MPID_PKT_MAX_DATA_SIZE));
...
#endif                  /* #DEBUG_END# */
/* Send as packet only */
CMMD_send_noblock(dest,MPID_PT2PT_TAG,(char*)(&pkt),sizeof(MPID_PKT_LONG_SYNC_T));
...
if (actual_len > MPID_PKT_DATA_SIZE) 
    return MPID_CMMD_post_send_long( dmpi_send_handle, mpid_send_handle, 
				   actual_len );
else
    return MPID_CMMD_post_send_short( dmpi_send_handle, mpid_send_handle, 
				    actual_len );
}
...
if (actual_len > MPID_PKT_DATA_SIZE) 
    ...
    return MPID_CMMD_post_send_long( dmpi_send_handle, mpid_send_handle, 
				   actual_len );
#else
    return MPID_CMMD_post_send_sync_long( dmpi_send_handle, mpid_send_handle, 
				        actual_len );
#endif
else
    return MPID_CMMD_post_send_sync_short( dmpi_send_handle, mpid_send_handle, 
					 actual_len );
}
