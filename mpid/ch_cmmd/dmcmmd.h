...
/*
 *  $Id: dmch.h,v 1.43 1996/01/11 18:35:15 gropp Exp gropp $
 *
...
#define MPIDTRANSPORT "TMC cm5/cmmd"
...
#include "cm/cmmd.h"
extern int __NUMNODES, __MYPROCID;static double __CMMD_DBL;extern double CMMD_node_timer_elapsed();typedef void *ProcSet;
/* #MEIKO DECLARATION# */
#else
/* 
...
/*
   ...
   to define a common data-structure that they could both access, and
...
typedef struct {
    ...
    int           bytes_as_contig;
        /* Rest of data */
    CMMD_mcb sid;              /* Id of non-blocking send, if used.
...
typedef struct {
    ...
    int           bytes_as_contig;
        /* Rest of data */
    CMMD_mcb rid;              /* Id of non-blocking recv, if used.
...
#define MPID_Post_send_sync(ctx,dmpi_send_handle) \
    MPID_CMMD_post_send_sync(dmpi_send_handle) 
...
#define MPID_Blocking_send(ctx, dmpi_send_handle) \
    MPID_CMMD_Blocking_send(dmpi_send_handle)
#else
#define MPID_Blocking_send(ctx, dmpi_send_handle) \
{int _err;\
...
#define MPID_Test_send( ctx, dmpi_send_handle ) \
    ((dmpi_send_handle)->completer == 0 ? \
     1 : MPID_CMMD_Test_send( dmpi_send_handle ))

#define MPID_Post_recv(ctx,dmpi_recv_handle ) \
    MPID_CMMD_post_recv(dmpi_recv_handle ) 
#define MPID_Blocking_recv(ctx,dmpi_recv_handle ) \
    MPID_CMMD_blocking_recv(dmpi_recv_handle) 
...
#define MPID_Test_recv( ctx, dmpi_recv_handle ) \
    ((dmpi_recv_handle)->completer == 0 ? \
        1 : MPID_CMMD_Test_recv_push( dmpi_recv_handle )) 
...
#define MPID_Test_request( ctx, request ) \
    ( (request)->chandle.handle_type == MPIR_SEND ? \
        MPID_CMMD_Test_send(&(request)->shandle) : \
        MPID_CMMD_Test_recv_push(&(request)->rhandle))
...
#define MPID_Check_device( ctx,blocking ) \
    MPID_CMMD_check_device( blocking )

#define MPID_Iprobe( ctx, tag, source, context_id, flag, status ) \
    MPID_CMMD_Iprobe( tag, source, context_id, flag, status ) 
#define MPID_Probe( ctx, tag, source, context_id, status ) \
    MPID_CMMD_Probe( tag, source, context_id, status ) 

#define MPID_NODE_NAME( ctx, name, len ) \
    MPID_CMMD_Node_name( name, len )
#define MPID_Version_name( ctx, name ) MPID_CMMD_Version_name( name )
#ifndef MPID_WTIME
#define MPID_WTIME(ctx)         MPID_CMMD_Wtime()
#endif
#define MPID_WTICK(ctx)         MPID_CMMD_Wtick()
#define MPID_INIT(argc,argv) MPID_CMMD_Init( argc, argv )
#define MPID_END(ctx)           MPID_CMMD_End()

#define MPID_ABORT( ctx, errorcode ) MPID_CMMD_Abort( errorcode );

#define MPID_CANCEL( ctx, r ) MPID_CMMD_Cancel( r )

#define MPID_Myrank( ctx, rank ) MPID_CMMD_Myrank( rank )
#define MPID_Mysize( ctx, size ) MPID_CMMD_Mysize( size )
...
#define MPID_Comm_init(ctx,comm,newcomm) MPID_CMMD_Comm_init(comm,newcomm)
#define MPID_Comm_free(ctx,comm) MPID_CMMD_Comm_free(comm)
#define MPID_Barrier(ctx,comm) MPID_CMMD_Barrier(comm)
...
#define MPID_Reduce_sum_int(ctx,send,recv,comm) \
      MPID_CMMD_Reduce_sum_int(send,recv,comm)
#define MPID_Reduce_sum_double(ctx,send,recv,comm) \
      MPID_CMMD_Reduce_sum_double(send,recv,comm)
...
#define MPID_Comm_init(ctx,comm,newcomm) MPID_CMMD_Comm_msgrep( newcomm )
#else
 ...
 */
#ifdef MPID_HAS_HETERO
#define MPID_PKT_PACK(pkt,size,dest) MPID_CMMD_Pkt_pack((MPID_PKT_T*)(pkt),size,dest)
#define MPID_PKT_UNPACK(pkt,size,src) MPID_CMMD_Pkt_unpack((MPID_PKT_T*)(pkt),size,src)
#else
...
#define MPID_Dest_byte_order(dest) MPID_CMMD_Dest_byte_order(dest)
#endif 
