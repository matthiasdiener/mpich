...
/* 
  ...
  a MPID_SPP_check_incoming routine that looks for headers.  Note that
...
/* them.                                                                   */
/***************************************************************************/
static int MPID_SPP_Copy_body_short( dmpi_recv_handle, pkt, pktbuf )
...
static int MPID_SPP_Copy_body_sync_short( dmpi_recv_handle, pkt, from )
MPIR_RHANDLE *dmpi_recv_handle;
MPID_PKT_T   *pkt;
int          from;
{
...
err = MPID_SPP_Copy_body_short( dmpi_recv_handle, pkt, 
 ...
 */
int MPID_SPP_post_recv( dmpi_recv_handle ) 
MPIR_RHANDLE *dmpi_recv_handle;
{
...
if (!dmpi_recv_handle->dev_rhandle.is_non_blocking) {
    return MPID_SPP_blocking_recv( dmpi_recv_handle );
    }
...
if (found) {
    ...
    return MPID_SPP_Process_unexpected_get( dmpi_recv_handle, 
					    dmpi_unexpected );
#endif
    }
 ...
 */
int MPID_SPP_Copy_body( dmpi_recv_handle, pkt, from )
MPIR_RHANDLE *dmpi_recv_handle;
MPID_PKT_T   *pkt;
int          from;
{
...
switch (pkt->head.mode) {
    ...
    err = MPID_SPP_Copy_body_sync_short( dmpi_recv_handle, pkt, from );
    DMPI_mark_recv_completed(dmpi_recv_handle);
    break;

#if defined(MPID_USE_GET)                    
    case MPID_PKT_DO_GET:
    MPID_SPP_Do_get( dmpi_recv_handle, from, (MPID_PKT_GET_T *)pkt );
 ...
 */
int MPID_SPP_Copy_body_unex( dmpi_recv_handle, pkt, from )
MPIR_RHANDLE *dmpi_recv_handle;
MPID_PKT_T   *pkt;
int          from;
{
...
switch (pkt->head.mode) {
	...
	MPID_SPP_Do_get_to_mem( address, from, (MPID_PKT_GET_T *)pkt );
...
/* dispatches them.  There is another such look in MPID_SPP_blocking_recv   */
 ...
 */    
int MPID_SPP_check_incoming( is_blocking )
...
/* Is the message expected or not? 
   ...
   is unexpected (is_posted == 0) */
	...
	switch (MPID_PKT_RECV_GET(pkt,head.mode)) {
	    case MPID_PKT_SHORT_READY:
	    case MPID_PKT_SHORT:
	    err = MPID_SPP_Copy_body_short( dmpi_recv_handle, 
					  MPID_PKT_RECV_ADDR(pkt), 
				     MPID_PKT_RECV_GET(pkt,short_pkt.buffer) );
	    break;
	case MPID_PKT_SHORT_SYNC:
	    err = MPID_SPP_Copy_body_sync_short( dmpi_recv_handle, 
					        MPID_PKT_RECV_ADDR(pkt), 
					        from );
	    break;
#ifdef MPID_USE_GET                    
        case MPID_PKT_DO_GET:
	    MPID_SPP_Do_get( dmpi_recv_handle, from, 
	...
	MPID_SPP_Copy_body_unex( dmpi_recv_handle, MPID_PKT_RECV_ADDR(pkt), 
			        from );
	}
    }
else {
...
#ifdef MPID_USE_GET                            
        case MPID_PKT_DONE_GET:
        MPID_SPP_Done_get( MPID_PKT_RECV_ADDR(pkt), from );
        break;
	case MPID_PKT_CONT_GET:
	MPID_SPP_Cont_get( MPID_PKT_RECV_ADDR(pkt), from );
	break;
#endif                                         
 ...
 */
int MPID_SPP_complete_recv( dmpi_recv_handle ) 
MPIR_RHANDLE *dmpi_recv_handle;
{
...
int MPID_SPP_Test_recv_push( dmpi_recv_handle )
MPIR_RHANDLE *dmpi_recv_handle;
{
...
/*
   ...
   complete-recv).
 */
int MPID_SPP_blocking_recv( dmpi_recv_handle ) 
MPIR_RHANDLE *dmpi_recv_handle;
{
MPID_PKT_RECV_DECL(MPID_PKT_T,pkt);
...
#endif                  /* #DEBUG_END# */
	if (is_posted) {
	    err = MPID_SPP_Copy_body( dmpi_recv_handle, 
