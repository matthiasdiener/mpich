 ...
 *****************************************************************************/
void MPID_SPP_Myrank( rank )
int *rank;
{
*rank = MPID_MyWorldRank;
}

void MPID_SPP_Mysize( size )
int *size;
{
*size = MPID_WorldSize;
}
...
MPID_SPP_Init_recv_code();
MPID_SPP_Init_send_code();
...
void MPID_SPP_Node_name( name, len )
char *name;
int  len;
{
...
void MPID_SPP_Version_name( name )
char *name;
{
...
cnt = 1000;
t1  = MPID_SPP_Wtime();
while (cnt-- && (t2 = MPID_SPP_Wtime()) <= t1) ;
if (!cnt) return 1.0e6;
return t2 - t1;
}

void MPID_SPP_Error_handler( r )
void (*r)();
{
if (r)
    MPID_ErrorHandler = r;
...
int MPID_SPP_Dest_byte_order( dest )
int dest;
{
if (MPID_IS_HETERO)
