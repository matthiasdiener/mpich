...
{if (MPID_DebugFlag) {\
    fprintf( MPID_DEBUG_FILE, \
	    "[%d]S About to copy to %#x from %#x (n=%d) (%s:%d)...\n", \
...
/* printf("pkt = %#x pkt->buffer = %#x\n", (int)pkt, (int)pkt->buffer); */
...
if (len > 0) {
    ...
    if (MPID_DebugFlag) {
	MPID_SHMEM_Print_pkt_data( "S Getting data from mpid->start", 
...
int MPID_SHMEM_post_send_long( dmpi_send_handle, mpid_send_handle, len ) 
MPIR_SHANDLE *dmpi_send_handle;
MPID_SHANDLE *mpid_send_handle;
int len;
{
...
#ifdef MPID_USE_GET
#else
int MPID_SHMEM_post_send_sync_long( dmpi_send_handle, mpid_send_handle, len ) 
MPIR_SHANDLE *dmpi_send_handle;
MPID_SHANDLE *mpid_send_handle;
int len;
{
...
if (actual_len > MPID_PKT_DATA_SIZE) 
    ...
    rc = MPID_SHMEM_post_send_long_rndv( dmpi_send_handle, mpid_send_handle, 
				      actual_len );
#else
    rc = MPID_SHMEM_post_send_long( dmpi_send_handle, mpid_send_handle, 
				   actual_len );
#endif
else
...
if (actual_len > MPID_PKT_DATA_SIZE) 
    ...
    rc = MPID_SHMEM_post_send_sync_long_get( dmpi_send_handle, mpid_send_handle, 
					  actual_len );
#else
    rc = MPID_SHMEM_post_send_sync_long( dmpi_send_handle, mpid_send_handle, 
				      actual_len );
#endif
else
