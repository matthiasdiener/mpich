...
/* Note that IBM MPL requires a struct for int in order to
   provide correctness:
if defined(eui) 
...
#define MPID_Blocking_send(ctx, dmpi_send_handle) \
    MPID_SPP_Blocking_send(dmpi_send_handle)
#else
#define MPID_Blocking_send(ctx, dmpi_send_handle) \
...
#define MPID_Test_send( ctx, dmpi_send_handle ) \
    ((dmpi_send_handle)->completer == 0 ? \
     1 : MPID_SPP_Test_send( dmpi_send_handle ))

#define MPID_Post_recv(ctx,dmpi_recv_handle ) \
    MPID_SPP_post_recv(dmpi_recv_handle ) 
#define MPID_Blocking_recv(ctx,dmpi_recv_handle ) \
    MPID_SPP_blocking_recv(dmpi_recv_handle) 
#define MPID_Complete_recv(ctx,dmpi_recv_handle) \
    MPID_SPP_complete_recv(dmpi_recv_handle) 
...
#define MPID_Test_recv( ctx, dmpi_recv_handle ) \
    ((dmpi_recv_handle)->completer == 0 ? \
        1 : MPID_SPP_Test_recv_push( dmpi_recv_handle )) 
...
#define MPID_Test_request( ctx, request ) \
    ( (request)->chandle.handle_type == MPIR_SEND ? \
        MPID_SPP_Test_send(&(request)->shandle) : \
        MPID_SPP_Test_recv_push(&(request)->rhandle))
...
#define MPID_Check_device( ctx,blocking ) \
    MPID_SPP_check_device( blocking )

#define MPID_Iprobe( ctx, tag, source, context_id, flag, status ) \
    MPID_SPP_Iprobe( tag, source, context_id, flag, status ) 
#define MPID_Probe( ctx, tag, source, context_id, status ) \
    MPID_SPP_Probe( tag, source, context_id, status ) 

#define MPID_NODE_NAME( ctx, name, len ) \
    MPID_SPP_Node_name( name, len )
#define MPID_Version_name( ctx, name ) MPID_SPP_Version_name( name )
#ifndef MPID_WTIME
#define MPID_WTIME(ctx)         MPID_SPP_Wtime()
#endif
#define MPID_WTICK(ctx)         MPID_SPP_Wtick()
#define MPID_INIT(argc,argv) MPID_SPP_Init( argc, argv )
#define MPID_END(ctx)           MPID_SPP_End()

#define MPID_ABORT( ctx, errorcode ) MPID_SPP_Abort( errorcode );

#define MPID_CANCEL( ctx, r ) MPID_SPP_Cancel( r )

#define MPID_Myrank( ctx, rank ) MPID_SPP_Myrank( rank )
#define MPID_Mysize( ctx, size ) MPID_SPP_Mysize( size )
...
#define MPID_Comm_init(ctx,comm,newcomm) MPID_SPP_Comm_init(comm,newcomm)
#define MPID_Comm_free(ctx,comm) MPID_SPP_Comm_free(comm)
#define MPID_Barrier(ctx,comm) MPID_SPP_Barrier(comm)
...
#define MPID_Reduce_sum_int(ctx,send,recv,comm) \
      MPID_SPP_Reduce_sum_int(send,recv,comm)
#define MPID_Reduce_sum_double(ctx,send,recv,comm) \
      MPID_SPP_Reduce_sum_double(send,recv,comm)
...
#define MPID_PKT_PACK(pkt,size,dest) MPID_SPP_Pkt_pack((MPID_PKT_T*)(pkt),size,dest)
#define MPID_PKT_UNPACK(pkt,size,src) MPID_SPP_Pkt_unpack((MPID_PKT_T*)(pkt),size,src)
#else
...
#define MPID_DRAIN_INCOMING \
    while (MPID_SPP_check_incoming( MPID_NOTBLOCKING ) != -1) ;
...
#define MPID_Dest_byte_order(dest) MPID_SPP_Dest_byte_order(dest)
#endif 
