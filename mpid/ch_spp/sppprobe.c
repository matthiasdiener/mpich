 ...
 */
int MPID_SPP_Iprobe( tag, source, context_id, found, status )
...
if (!*found) {
    ...
    MPID_SPP_check_incoming( MPID_NOTBLOCKING );
    DMPI_search_unexpected_queue( source, tag, context_id, 
				  found, 0, &dmpi_unexpected );
    }
if (*found) {
...
void MPID_SPP_Probe( tag, source, context_id, status )
...
while (1) {
    MPID_SPP_Iprobe( tag, source, context_id, &found, status );
    if (found) break;
    /* Wait for a message */
    MPID_SPP_check_incoming( MPID_BLOCKING );
    }
