...
/* Send a short (single packet message) 
   ...
   macros expect dest) 
*/
int MPID_SPP_send_short( buf, len, tag, context_id, lrank_sender, dest,
		        msgrep ) 
void *buf;
...
int MPID_SPP_post_send_short( dmpi_send_handle, mpid_send_handle, len ) 
...
int MPID_SPP_post_send_sync_short( dmpi_send_handle, mpid_send_handle, len ) 
MPIR_SHANDLE *dmpi_send_handle;
MPID_SHANDLE *mpid_send_handle;
int len;
{
...
MPID_PKT_SEND_SET(pkt,sync_id,
		  MPID_SPP_Get_Sync_Id( dmpi_send_handle, mpid_send_handle ));
...
void MPID_SPP_Cmpl_send_sync( dmpi_send_handle )
MPIR_SHANDLE *dmpi_send_handle;
{
...
while (!MPID_Test_handle(dmpi_send_handle)) {
    ...
    (void)MPID_SPP_check_incoming( MPID_BLOCKING );
    }
...
int len;
{
MPID_SPP_post_send_long_rndv( dmpi_send_handle, mpid_send_handle, len );
}
#endif  /* else of non-rndv sync send */

#ifdef MPID_ADI_MUST_SENDSELF
/****************************************************************************
  MPID_SPP_Post_send_local
 ...
 ***************************************************************************/
int MPID_SPP_Post_send_local( dmpi_send_handle, mpid_send_handle, len )
MPIR_SHANDLE *dmpi_send_handle;
MPID_SHANDLE *mpid_send_handle;
int           len;
{
 ...
 */
int MPID_SPP_post_send( dmpi_send_handle ) 
MPIR_SHANDLE *dmpi_send_handle;
{
MPID_SHANDLE *mpid_send_handle;
int         actual_len, rc;
...
if (dest == MPID_MyWorldRank) {
    return MPID_SPP_Post_send_local( dmpi_send_handle, mpid_send_handle, 
				    actual_len );
    }
 }
#endif
...
#ifdef MPID_USE_GET
    rc = MPID_SPP_post_send_long_get( dmpi_send_handle, mpid_send_handle, 
				   actual_len );
#endif
else
    rc = MPID_SPP_post_send_short( dmpi_send_handle, mpid_send_handle, 
				    actual_len );
...
int MPID_SPP_post_send_sync( dmpi_send_handle ) 
MPIR_SHANDLE *dmpi_send_handle;
{
MPID_SHANDLE *mpid_send_handle;
int         actual_len, rc;
...
if (actual_len > MPID_PKT_DATA_SIZE) 
    ...
    rc = MPID_SPP_post_send_sync_long_get( dmpi_send_handle, mpid_send_handle, 
					  actual_len );
#endif
else
    rc = MPID_SPP_post_send_sync_short( dmpi_send_handle, mpid_send_handle, 
					 actual_len );
 ...
 */
int MPID_SPP_Contig_blocking_send( buf, len, tag, context_id, lrank_sender, 
				  grank_dest, msgrep )
void *buf;
...
if (len <= MPID_PKT_DATA_SIZE) 
    rc = MPID_SPP_send_short( buf, len, tag, context_id, lrank_sender, 
			     grank_dest, msgrep );
else 
#ifdef MPID_USE_GET
    rc = MPID_SPP_send_long_get( buf, len, tag, context_id, lrank_sender, 
			        grank_dest, msgrep );
#ifdef FOO
#endif
#else
.... unsupported ... (part of ifdef FOO)
#endif
...
int MPID_SPP_Blocking_send( dmpi_send_handle )
MPIR_SHANDLE *dmpi_send_handle;
{
 ...
 */
int MPID_SPP_complete_send( dmpi_send_handle ) 
MPIR_SHANDLE *dmpi_send_handle;
{
...
switch (dmpi_send_handle->completer) {
    ...
    case MPID_CMPL_SEND_GET:
	 MPID_SPP_Cmpl_send_get( dmpi_send_handle );
         break;
#endif                                 
#ifndef MPID_USE_RNDV
    case MPID_CMPL_SEND_SYNC:
	 /* Also does non-blocking sync sends */
	 MPID_SPP_Cmpl_send_sync( dmpi_send_handle );
	 break;
#endif
    default:
 ...
 */
int MPID_SPP_Test_send( dmpi_send_handle )
MPIR_SHANDLE *dmpi_send_handle;
{
/* Need code for GET? */
