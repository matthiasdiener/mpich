...
#define DEBUG_PRINT_SMP_SEND_PKT(msg,pkt)\
{  if (MPID_DebugFlag) {\
     SPRINTF( ch_debug_buf, \
              "[%d]%s of tag = %d, dest = %d, ctx = %d, len = %d, mode = ", \
           MPID_MyWorldRank, msg, (pkt)->tag, destination, \
           (pkt)->context_id, (pkt)->len );\
     if (MPID_UseDebugFile) {\
       fputs( ch_debug_buf, MPID_DEBUG_FILE );\
       SMPI_Print_mode( MPID_DEBUG_FILE, (SMPI_PKT_T *)(pkt) );\
       FPRINTF( MPID_DEBUG_FILE, "(%s:%d)\n", __FILE__, __LINE__ );\
       fflush( MPID_DEBUG_FILE );}\
     MPID_SAVE_MSG;}}
...
#define DEBUG_PRINT_SMP_RECV_PKT(msg,pkt)\
{  if (MPID_DebugFlag) {\
     SPRINTF( ch_debug_buf,\
            "[%d]%s for tag = %d, source = %d, ctx = %d, len = %d, mode = ", \
...
#define DEBUG_PRINT_SMP_PKT(msg,pkt) \
{  if (MPID_DebugFlag) {\
     SPRINTF( ch_debug_buf,\
           "[%d]%s (%s:%d)\n", MPID_MyWorldRank, msg, __FILE__, __LINE__ );\
     if (MPID_UseDebugFile) {\
       fputs( ch_debug_buf, MPID_DEBUG_FILE );\
       SMPI_Print_packet( MPID_DEBUG_FILE, (pkt) );\
       fflush( MPID_DEBUG_FILE );}\
     MPID_SAVE_MSG;}}
...
#define DEBUG_PRINT_PKT_DATA(msg,pkt)\
    if (MPID_DebugFlag) {\
      if (MPID_UseDebugFile) {\
	MPID_Print_pkt_data( msg, \
          (void *)((unsigned long)pkt+sizeof(MPID_PKT_SHORT_T)), len );}\
	}
