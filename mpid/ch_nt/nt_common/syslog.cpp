...
int add_source(char* source,int facility){
	...
	if(facility==LOG_APP){ strncpy(reg_key,APP_LOG_PATH,_MAX_PATH); }
	else if(facility==LOG_SYS){	strncpy(reg_key,SYS_LOG_PATH,_MAX_PATH); }
	else {
	#ifdef DEBUG_SYSLOG
...
int add_event(int priority,char* logmsg){
	...
	switch(priority){
		case LOG_INFO: event_type=EVENTLOG_INFORMATION_TYPE;break;
		case LOG_ERR: event_type=EVENTLOG_ERROR_TYPE;break;
		case LOG_WARNING: event_type=EVENTLOG_WARNING_TYPE;break;
		default: {
			#ifdef DEBUG_SYSLOG
