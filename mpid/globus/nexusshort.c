...
int MPID_Nexus_send_short(void *buf, int len,
			  int src_lrank, int tag, int context_id,
			  int dest, int msgrep);
int MPID_Nexus_isend_short (void *buf, int len,
			    int src_lrank, int tag, int context_id,
			    int dest, int msgrep,
			    MPIR_SHANDLE *shandle);
int MPID_Nexus_recv_short (MPIR_RHANDLE *rhandle,
			   int from_grank,
			   void *in_pkt);
int MPID_Nexus_save_short (MPIR_RHANDLE *rhandle,
			   int from,
			   void *in_pkt);
int MPID_Nexus_unxrecv_start_short (MPIR_RHANDLE *rhandle,
				    void *in_runex);
void MPID_Nexus_short_delete (MPID_Protocol *p);
MPID_Protocol MPID_Nexus_Short_setup(void);
/*
...
void         *in_pkt;
{
    ...
    if (rhandle->finish) {
	MPID_DO_HETERO(rhandle->msgrep = pkt->msgrep);
