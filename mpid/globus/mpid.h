...
/* needed for SET_STATUSCOUNT_ISZERO((status)->private_count); */
#include "nexuspriv.h"

/* begin NICK */
/* MPID_HAS_GET_ELEMENTS and MPID_HAS_GET_COUNT are defined    */
/* to use the Nexus-device-supplied versions of these routines */
 ...
 * change this)
 */
#define MPID_ZERO_STATUS_COUNT(status) \
    { \
	(status)->count = 0; \
	SET_STATUSCOUNT_ISZERO((status)->private_count); \
    }
