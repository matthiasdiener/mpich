...
fprintf(stderr, "NICK: %d entering globus2 barrier\n", MPID_MyWorldRank);
...
fprintf(stderr, "NICK: %d globus2 barrier: 1st loop set.level %d\n", MPID_MyWorldRank, set.level);
...
fprintf(stderr, "NICK: %d globus2 barrier: before 1st wait_barrier n_req %d\n", MPID_MyWorldRank, n_req);
   /* start of EXIT_FUZZY_BARRIER */
   /* wait for my neighbors to enter the barrier */
   mpi_errno = wait_barrier(req, n_req);
fprintf(stderr, "NICK: %d globus2 barrier: after 1st wait_barrier n_req %d\n", MPID_MyWorldRank, n_req);
   if ( mpi_errno ) goto clean_exit;
   ...
   {
      single_set_t set = set_info.sets[lvl];
fprintf(stderr, "NICK: %d globus2 barrier: 2nd loop set.level %d\n", MPID_MyWorldRank, set.level);
...
fprintf(stderr, "NICK: %d globus2 barrier: before 2nd wait_barrier n_req %d\n", MPID_MyWorldRank, n_req);
   /* exit the barrier (waiting again) */
   mpi_errno = wait_barrier(req, n_req);
   /* end of EXIT_FUZZY_BARRIER */
fprintf(stderr, "NICK: %d globus2 barrier: after wait_barrier\n", MPID_MyWorldRank);
...
fprintf(stderr, "NICK: %d exiting globus2 barrier\n", MPID_MyWorldRank);
   return (mpi_errno);
}   /* MPID_FN_Barrier */
#endif   /* MPID_Barrier */
