...
#define MPID_MSG_LEN(pkt) \
    ((pkt)->mode & MPID_MODE_LONG ? (ptk)->len : __P4LEN - MPID_HEADER_LEN)
...
#define MPID_PKT_INIT() \
    pkt = (MPID_PKT_T *)p4_msg_alloc(sizeof(MPID_PKT_T));
#define MPID_PKT_CHECK()  \
    (__P4TYPE=MPID_PT2PT_TAG,__P4FROM= -1,p4_messages_available(&__P4TYPE,&__P4FROM))
#define MPID_PKT_WAIT() MPID_PKT_POST_AND_WAIT()
#define MPID_PKT_POST() 
#define MPID_PKT_POST_AND_WAIT() \
    {__P4LEN=sizeof(MPID_PKT_T);__P4FROM= -1;__P4TYPE=MPID_PT2PT_TAG;p4_recv(&__P4TYPE,&__P4FROM,(char**)&(pkt),&__P4LEN);}
#define MPID_PKT_FREE() \
    p4_msg_free(pkt)
#define MPID_PKT (*pkt)
