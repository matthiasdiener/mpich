...
void MPID_P4_Free_barrier(vbar)
void *vbar;
{
...
MPID_P4_Setup_barrier(bar,mypidf)
MPID_Fastbar *bar;
int mypidf;
{
    int mypid,np;
...
double MPID_P4_Barrier_sum_double(bar,x)
MPID_Fastbar *bar;
double *x;
{
    register int oldphase;
...
double MPID_P4_Barrier_max_double(bar,x)
MPID_Fastbar *bar;
double *x;
{
    register int oldphase;
...
double MPID_P4_Barrier_min_double(bar,x)
MPID_Fastbar *bar;
double *x;
{
    register int oldphase;
...
char **argv;
{
    ...
    bar = MPID_P4_Init_barrier(n,1);
    MPID_P4_Setup_barrier(bar,myid);
    ...
    starttime = p4_ustimer();
    for (i = 0; i < 1000; i++)
      ysum = MPID_P4_Barrier_sum_double(bar,&x);
    endtime = p4_ustimer();
    sumtime = endtime - starttime;

    MPID_P4_Free_barrier( bar );
    p4_wait_for_end();
