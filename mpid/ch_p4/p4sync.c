...
/*
 *  $Id: chsync.c,v 1.10 1994/10/24 22:03:27 gropp Exp gropp $
 *
...
#ifndef lint
static char vc[] = "$Id: chsync.c,v 1.10 1994/10/24 22:03:27 gropp Exp gropp $";
#endif
...
MPID_Aint MPID_P4_Get_Sync_Id( dmpi_handle, mpid_handle )
...
new		      = (MPID_SyncId *)malloc(sizeof(MPID_SyncId));    /* This should use SBalloc code */
if (!new) MPID_P4_Abort( MPI_ERR_EXHAUSTED );
new->id		      = CurId++;
...
int MPID_P4_Lookup_SyncAck( sync_id, dmpi_send_handle, mpid_send_handle )
int          sync_id;
MPIR_SHANDLE **dmpi_send_handle;
MPID_SHANDLE **mpid_send_handle;
{
...
if (DebugFlag) {
    ...
    "[%d]SYNC received sync ack message for mode=%x from %d (%s:%d)\n",  
	   __MYPROCID, sync_id, from, __FILE__, __LINE__ );
    fflush( stdout );
    }
#endif                  /* #DEBUG_END# */
MPID_P4_Lookup_SyncAck( sync_id, &dmpi_send_handle, &mpid_send_handle );
if (!dmpi_send_handle) {
...
if (DebugFlag) {
   ...
   "[%d]SYNC Starting a send of tag = %d, dest = %d, mode=",
	    __MYPROCID, MPID_PT2PT_TAG, from );
...
#endif                  /* #DEBUG_END# */
p4_sendx(MPID_PT2PT_TAG,from,(char *)(&pkt),sizeof(MPID_PKT_SYNC_ACK_T),P4NOX );
}
