...
/* 
  ...
  a MPID_P4_check_incoming routine that looks for headers.  Note that
...
/* buffer.                                                                 */
/***************************************************************************/
int MPID_P4_Copy_body_short( dmpi_recv_handle, pkt, pktbuf )
...
int MPID_P4_Copy_body_sync_short( dmpi_recv_handle, pkt, from )
MPIR_RHANDLE *dmpi_recv_handle;
MPID_PKT_T   *pkt;
int          from;
{
...
err = MPID_P4_Copy_body_short( dmpi_recv_handle, pkt, 
...
/* 
    ...
    sender for the data...
 */
int MPID_P4_Copy_body_long( dmpi_recv_handle, pkt, from )
MPIR_RHANDLE *dmpi_recv_handle;
MPID_PKT_T   *pkt;
int          from;
{
...
{char *__p4lbuf=0;__P4LEN=msglen;__P4FROM= -1;__P4TYPE=MPID_PT2PT2_TAG(from);p4_recv(&__P4TYPE,&__P4FROM,&__p4lbuf,&__P4LEN);memcpy(mpid_recv_handle->start,__p4lbuf,__P4LEN);p4_msg_free(__p4lbuf);};
DMPI_mark_recv_completed(dmpi_recv_handle);

return err;
}

int MPID_P4_Copy_body_sync_long( dmpi_recv_handle, pkt, from )
MPIR_RHANDLE *dmpi_recv_handle;
MPID_PKT_T   *pkt;
int          from;
{
...
err = MPID_P4_Copy_body_long( dmpi_recv_handle, pkt, from );
 ...
 */
int MPID_P4_Process_unexpected( dmpi_recv_handle, dmpi_unexpected )
...
if (mpid_recv_handle_unex->send_id) {
    MPID_P4_Ack_Request( mpid_recv_handle, mpid_recv_handle_unex->from,
...
/*
   ...
   processing.
 */
int MPID_P4_post_recv( dmpi_recv_handle, is_available ) 
MPIR_RHANDLE *dmpi_recv_handle;
int          *is_available;
{
...
if (!dmpi_recv_handle->dev_rhandle.is_non_blocking) {
    *is_available = 1;
    return MPID_P4_blocking_recv( dmpi_recv_handle );
    }
...
#endif                  /* #DEBUG_END# */
    *is_available = 1;
    return MPID_P4_Process_unexpected( dmpi_recv_handle, dmpi_unexpected );
    }
*is_available    = 0;
 ...
 */
int MPID_P4_Copy_body( dmpi_recv_handle, pkt, from )
MPIR_RHANDLE *dmpi_recv_handle;
MPID_PKT_T   *pkt;
int          from;
{
...
switch (pkt->head.mode) {
    ...
    err = MPID_P4_Copy_body_sync_short( dmpi_recv_handle, pkt, from );
    DMPI_mark_recv_completed(dmpi_recv_handle);
    break;
    ...
    /* Send back an OK to send, with a tag value and 
       a posted recv */
    MPID_P4_Ack_Request( &dmpi_recv_handle->dev_rhandle, from, 
    ...
    err = MPID_P4_Copy_body_sync_long( dmpi_recv_handle, pkt, from );
    DMPI_mark_recv_completed(dmpi_recv_handle);
    break;
#endif
    }
...
int MPID_P4_Copy_body_unex( dmpi_recv_handle, pkt, from )
MPIR_RHANDLE *dmpi_recv_handle;
MPID_PKT_T   *pkt;
int          from;
{
...
switch (pkt->head.mode) {
	...
	MPIDGETMEM(msglen);
	MPID_KEEP_STAT(n_long++;)
	{char *__p4lbuf=0;__P4LEN=msglen;__P4FROM= -1;__P4TYPE=MPID_PT2PT2_TAG(from);p4_recv(&__P4TYPE,&__P4FROM,&__p4lbuf,&__P4LEN);memcpy(address,__p4lbuf,__P4LEN);p4_msg_free(__p4lbuf);};
	break;
#endif
    }
...
/* dispatches them.  There is another such look in MPID_P4_blocking_recv   */
 ...
 */    
int MPID_P4_check_incoming( is_blocking )
...
if (is_blocking == MPID_NOTBLOCKING) {
    ...
    {char *__p4lbuf=0;__P4LEN=sizeof(MPID_PKT_T);__P4FROM= -1;__P4TYPE=MPID_PT2PT_TAG;p4_recv(&__P4TYPE,&__P4FROM,&__p4lbuf,&__P4LEN);memcpy(&pkt,__p4lbuf,__P4LEN);p4_msg_free(__p4lbuf);};
    }
}       /* #PVM3_END# */
#endif
...
if (DebugFlag) {
	...
	switch (MPID_PKT.head.mode) {
	    case MPID_PKT_SHORT_READY:
	    case MPID_PKT_SHORT:
	    err = MPID_P4_Copy_body_short( dmpi_recv_handle, &MPID_PKT, 
	...
	case MPID_PKT_LONG_READY:
	case MPID_PKT_LONG:
	    err = MPID_P4_Copy_body_long( dmpi_recv_handle, &MPID_PKT, from );
	    break;
	case MPID_PKT_LONG_SYNC:
	    err = MPID_P4_Copy_body_sync_long( dmpi_recv_handle, &MPID_PKT, 
					       from );
	    break;
#endif
	case MPID_PKT_SHORT_SYNC:
	    err = MPID_P4_Copy_body_sync_short( dmpi_recv_handle, &MPID_PKT, 
					        from );
	    break;
	    }
	}
    else {
	MPID_P4_Copy_body_unex( dmpi_recv_handle, &MPID_PKT, from );
	}
    }
else {
    switch (MPID_PKT.head.mode) {
	...
	MPID_P4_Do_Request( MPID_PKT.sendok_pkt.use_tag, from, 
...
/*
   ...
   queue.
 */
int MPID_P4_blocking_recv( dmpi_recv_handle ) 
MPIR_RHANDLE *dmpi_recv_handle;
{
MPID_PKT_LALLOC
...
#endif                  /* #DEBUG_END# */
	if (is_posted) {
	    err = MPID_P4_Copy_body( dmpi_recv_handle, &MPID_PKT, from );
#ifdef MPID_USE_RNDV
...
/* Respond to a request to send a message when the message is found to
   be posted */
int MPID_P4_Ack_Request( mpid_recv_handle, from, send_id )
MPID_RHANDLE *mpid_recv_handle;
int          from;
MPID_Aint    send_id;
{
...
p4_sendx(MPID_PT2PT_TAG,from,(char *)(&pkt),sizeof(MPID_PKT_OK_TO_SEND_T),P4NOX );
return MPI_SUCCESS;
}

MPID_P4_Complete_Rndv( mpid_recv_handle )
MPID_RHANDLE *mpid_recv_handle;
{
if (--TagsInUse == 0) CurTag = 1;
{char *__p4lbuf=0;__P4LEN=0;__P4FROM= -1;__P4TYPE=0;p4_recv(&__P4TYPE,&__P4FROM,&__p4lbuf,&__P4LEN);memcpy(0,__p4lbuf,__P4LEN);p4_msg_free(__p4lbuf);};
mpid_recv_handle->rid = 0;
}

/* Fullfill a request for a message */
int MPID_P4_Do_Request( use_tag, from, send_id )
int          use_tag, from;
MPID_Aint    send_id;
{
...
{mpid_send_handle->sid =0;p4_sendx(use_tag,from,(char *)(dmpi_send_handle->dev_shandle.start),dmpi_send_handle->dev_shandle.bytes_as_contig,P4NOX);};
return MPI_SUCCESS;
}
#endif
