...
#define MPID_Test_send( ctx, dmpi_send_handle ) \
    ((dmpi_send_handle)->completer == 0 ? \
     1 : MPID_P4_Test_send( dmpi_send_handle ))

#define MPID_Post_recv(ctx,dmpi_recv_handle ) \
    MPID_P4_post_recv(dmpi_recv_handle ) 
...
#define MPID_Test_recv( ctx, dmpi_recv_handle ) \
    ((dmpi_recv_handle)->completer == 0 ? \
        1 : MPID_P4_Test_recv_push( dmpi_recv_handle )) 
...
#define MPID_Test_request( ctx, request ) \
    ( (request)->chandle.handle_type == MPIR_SEND ? \
        MPID_P4_Test_send(&(request)->shandle) : \
        MPID_P4_Test_recv_push(&(request)->rhandle))
...
#define MPID_Comm_init(ctx,comm,newcomm) MPID_P4_Comm_init(comm,newcomm)
 ...
 */
#define MPID_P4K_MSGLEN(dmpi_recv_handle,msglen,err) \
...
#define MPID_PKT_PACK(pkt,size,dest) MPID_P4_Pkt_pack(pkt,size,dest)
#define MPID_PKT_UNPACK(pkt,size,src) MPID_P4_Pkt_unpack(pkt,size,src)
#else
