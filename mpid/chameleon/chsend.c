/*
 *  $Id: chsend.c,v 1.37 1995/09/29 22:34:19 gropp Exp $
 *
...
#ifndef lint
static char vcid[] = "$Id: chsend.c,v 1.37 1995/09/29 22:34:19 gropp Exp $";
#endif
...
/* Send a short (single packet message) 
   ...
   macros expect dest) 
*/
int MPID_CH_send_short( buf, len, tag, context_id, lrank_sender, dest,
		        msgrep ) 
void *buf;
...
int MPID_CH_post_send_long_eager( dmpi_send_handle, mpid_send_handle, len ) 
MPIR_SHANDLE *dmpi_send_handle;
MPID_SHANDLE *mpid_send_handle;
int len;
{
...
#ifdef MPID_USE_GET               /*#NONGET_START#*/
#else
int MPID_CH_post_send_sync_long_eager( dmpi_send_handle, mpid_send_handle, 
				       len ) 
MPIR_SHANDLE *dmpi_send_handle;
MPID_SHANDLE *mpid_send_handle;
int len;
...
void MPID_CH_Cmpl_send_sync( dmpi_send_handle )
MPIR_SHANDLE *dmpi_send_handle;
{
MPID_SHANDLE *mpid_send_handle = &dmpi_send_handle->dev_shandle;
...
int len;
{
MPID_CH_post_send_long_rndv( dmpi_send_handle, mpid_send_handle, len );
}
                                           /*#NONGET_END#*/
int MPID_CH_post_send_sync_short( dmpi_send_handle, mpid_send_handle, len ) 
MPIR_SHANDLE *dmpi_send_handle;
...
if (dest == MPID_MyWorldRank) {
    return MPID_CH_Post_send_local( dmpi_send_handle, mpid_send_handle, 
				    actual_len );
    }
 }
#endif
...
if (actual_len > MPID_PKT_DATA_SIZE) 
    ...
    rc = MPID_CH_post_send_long_rndv( dmpi_send_handle, mpid_send_handle, 
				      actual_len );
#else
    rc = MPID_CH_post_send_long_eager( dmpi_send_handle, mpid_send_handle, 
				       actual_len );
                                          /*#NONGET_END#*/
#endif
else
...
if (actual_len > MPID_PKT_DATA_SIZE) 
    ...
    rc = MPID_CH_post_send_long_rndv( dmpi_send_handle, mpid_send_handle, 
				      actual_len );
#else
    rc = MPID_CH_post_send_sync_long_eager( dmpi_send_handle, 
 ...
 */
int MPID_CH_Contig_blocking_send( buf, len, tag, context_id, lrank_sender, 
				  grank_dest, msgrep )
void *buf;
...
if (len <= MPID_PKT_DATA_SIZE) 
    rc = MPID_CH_send_short( buf, len, tag, context_id, lrank_sender, 
			     grank_dest, msgrep );
else 
#ifdef MPID_USE_GET
    rc = MPID_CH_send_long_get( buf, len, tag, context_id, lrank_sender, 
			        grank_dest, msgrep );
#ifdef FOO
#elif defined(MPID_USE_RNDV)              /*#NONGET_START#*/
    rc = MPID_CH_send_long_rndv( buf, len, tag, context_id, lrank_sender, 
			         grank_dest, msgrep);
#else
    rc = MPID_CH_send_long_eager( buf, len, tag, context_id, lrank_sender, 
				  grank_dest, msgrep);
                                          /*#NONGET_END#*/
#endif
#else
...
{
#ifdef MPID_USE_RNDV                           /*#NONGET_START#*/
MPID_CH_Test_send_rndv( dmpi_send_handle );
#endif                                         
#ifndef PI_NO_NSEND
