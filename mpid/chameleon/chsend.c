...
if (dmpi_send_handle->msgrep == MPIR_MSGREP_XDR) 
    pkt.has_xdr = MPID_MODE_XDR;
#endif                         /* #HETERO_END# */
pkt.sync_id      = MPID_CH_Get_Sync_Id( dmpi_send_handle, mpid_send_handle );
...
if (actual_len > MPID_PKT_DATA_SIZE) 
    return MPID_CH_post_send_long( dmpi_send_handle, mpid_send_handle, 
				   actual_len );
else
    return MPID_CH_post_send_short( dmpi_send_handle, mpid_send_handle, 
				    actual_len );
}
...
if (actual_len > MPID_PKT_DATA_SIZE) 
    ...
    return MPID_CH_post_send_long( dmpi_send_handle, mpid_send_handle, 
				   actual_len );
#else
    return MPID_CH_post_send_sync_long( dmpi_send_handle, mpid_send_handle, 
				        actual_len );
#endif
else
    return MPID_CH_post_send_sync_short( dmpi_send_handle, mpid_send_handle, 
					 actual_len );
}
