...
#define MPIDTRANSPORT "Chameleon"
...
#define MPID_Post_send_sync(ctx,dmpi_send_handle) \
    MPID_CH_post_send_sync(dmpi_send_handle) 
...
#define MPID_Test_send( ctx, dmpi_send_handle ) \
    ((dmpi_send_handle)->completer == 0 ? \
     1 : MPID_CH_Test_send( dmpi_send_handle ))

#define MPID_Post_recv(ctx,dmpi_recv_handle ) \
    MPID_CH_post_recv(dmpi_recv_handle ) 
...
#define MPID_Test_recv( ctx, dmpi_recv_handle ) \
    ((dmpi_recv_handle)->completer == 0 ? \
        1 : MPID_CH_Test_recv_push( dmpi_recv_handle )) 
...
#define MPID_Test_request( ctx, request ) \
    ( (request)->chandle.handle_type == MPIR_SEND ? \
        MPID_CH_Test_send(&(request)->shandle) : \
        MPID_CH_Test_recv_push(&(request)->rhandle))
...
#define MPID_Iprobe( ctx, tag, source, context_id, flag, status ) \
    MPID_CH_Iprobe( tag, source, context_id, flag, status ) 
#define MPID_Probe( ctx, tag, source, context_id, status ) \
    MPID_CH_Probe( tag, source, context_id, status ) 

#define MPID_NODE_NAME( ctx, name, len ) \
    MPID_CH_Node_name( name, len )
#define MPID_Version_name( ctx, name ) MPID_CH_Version_name( name )
#ifndef MPID_WTIME
#define MPID_WTIME(ctx)         MPID_CH_Wtime()
#endif
#define MPID_WTICK(ctx)         MPID_CH_Wtick()
#define MPID_INIT(argc,argv) MPID_CH_Init( argc, argv )
#define MPID_END(ctx)           MPID_CH_End()

#define MPID_ABORT( ctx, errorcode ) MPID_CH_Abort( errorcode );

#define MPID_CANCEL( ctx, r ) MPID_CH_Cancel( r )

#define MPID_Myrank( ctx, rank ) MPID_CH_Myrank( rank )
#define MPID_Mysize( ctx, size ) MPID_CH_Mysize( size )
...
#define MPID_Reduce_sum_double(ctx,send,recv,comm) \
      MPID_CH_Reduce_sum_double(send,recv,comm)
...
#define MPID_Comm_init(ctx,comm,newcomm) MPID_CH_Comm_msgrep( newcomm )
#else
...
#define MPID_Dest_byte_order(dest) MPID_CH_Dest_byte_order(dest)
#endif 
