/*
 *  $Id: chrecv.c,v 1.31 1994/11/23 16:04:10 gropp Exp $
 *
...
#ifndef lint
static char vcid[] = "$Id: chrecv.c,v 1.31 1994/11/23 16:04:10 gropp Exp $";
#endif /* lint */
...
fprintf( stdout, "[%d] short = %d, long = %d, unexpected = %d, ack = %d\n",
	 PImytid, n_short, n_long, n_unexpected, n_syncack );
#endif
}
/***************************************************************************/
...
/* buffer.                                                                 */
/***************************************************************************/
int MPID_CH_Copy_body_short( dmpi_recv_handle, pkt, pktbuf )
...
int MPID_CH_Copy_body_sync_short( dmpi_recv_handle, pkt, from )
MPIR_RHANDLE *dmpi_recv_handle;
MPID_PKT_T   *pkt;
int          from;
{
...
err = MPID_CH_Copy_body_short( dmpi_recv_handle, pkt, 
			       pkt->short_sync_pkt.buffer );

#ifdef MPID_DEBUG_ALL   /* #DEBUG_START# */
if (DebugFlag) {
    printf( "[%d]SYNC Returning sync to %d with mode ", PImytid,
	   from );
...
/* 
    ...
    sender for the data...
 */
int MPID_CH_Copy_body_long( dmpi_recv_handle, pkt, from )
MPIR_RHANDLE *dmpi_recv_handle;
MPID_PKT_T   *pkt;
int          from;
{
...
PIbrecv( MPID_PT2PT2_TAG(from), mpid_recv_handle->start, msglen, MSG_OTHER );
DMPI_mark_recv_completed(dmpi_recv_handle);

return err;
}

int MPID_CH_Copy_body_sync_long( dmpi_recv_handle, pkt, from )
MPIR_RHANDLE *dmpi_recv_handle;
MPID_PKT_T   *pkt;
int          from;
{
...
err = MPID_CH_Copy_body_long( dmpi_recv_handle, pkt, from );
 ...
 */
int MPID_CH_Process_unexpected( dmpi_recv_handle, dmpi_unexpected )
...
if (DebugFlag) {
    printf( "[%d]R Found message in temp area of %d bytes (%s:%d)...\n", 
	    PImytid, mpid_recv_handle_unex->bytes_as_contig,
	    __FILE__, __LINE__ );
    fflush( stdout );
    }
#endif                  /* #DEBUG_END# */
...
if (mpid_recv_handle_unex->send_id) {
    MPID_CH_Ack_Request( mpid_recv_handle, mpid_recv_handle_unex->from,
...
if (DebugFlag) {
  ...
  "[%d]R Copied message out of temp area; send mode is %x (%s:%d)..\n", 
	    PImytid, mpid_recv_handle_unex->mode, __FILE__, __LINE__ );
    fflush( stdout );
    }
#endif                  /* #DEBUG_END# */
...
/*
   ...
   processing.
 */
int MPID_CH_post_recv( dmpi_recv_handle, is_available ) 
MPIR_RHANDLE *dmpi_recv_handle;
int          *is_available;
{
...
if (!dmpi_recv_handle->dev_rhandle.is_non_blocking) {
    *is_available = 1;
    return MPID_CH_blocking_recv( dmpi_recv_handle );
    }
...
if (DebugFlag) {
    printf( 
      "[%d]R starting recv for tag = %d, source = %d, ctx = %d (%s:%d)\n", 
	    PImytid, dmpi_recv_handle->tag, dmpi_recv_handle->source,
...
#endif                  /* #DEBUG_END# */
    *is_available = 1;
    return MPID_CH_Process_unexpected( dmpi_recv_handle, dmpi_unexpected );
    }
*is_available    = 0;
 ...
 */
int MPID_CH_Copy_body( dmpi_recv_handle, pkt, from )
MPIR_RHANDLE *dmpi_recv_handle;
MPID_PKT_T   *pkt;
int          from;
{
...
switch (pkt->head.mode) {
    ...
    err = MPID_CH_Copy_body_sync_short( dmpi_recv_handle, pkt, from );
    DMPI_mark_recv_completed(dmpi_recv_handle);
    break;
    ...
    /* Send back an OK to send, with a tag value and 
       a posted recv */
    MPID_CH_Ack_Request( &dmpi_recv_handle->dev_rhandle, from, 
    ...
    err = MPID_CH_Copy_body_sync_long( dmpi_recv_handle, pkt, from );
    DMPI_mark_recv_completed(dmpi_recv_handle);
    break;
#endif
    }
...
int MPID_CH_Copy_body_unex( dmpi_recv_handle, pkt, from )
MPIR_RHANDLE *dmpi_recv_handle;
MPID_PKT_T   *pkt;
int          from;
{
...
switch (pkt->head.mode) {
	...
	MPIDGETMEM(msglen);
	MPID_KEEP_STAT(n_long++;)
	PIbrecv( MPID_PT2PT2_TAG(from), address, msglen, MSG_OTHER );
	break;
#endif
    }
 ...
 */    
int MPID_CH_check_incoming( is_blocking )
...
if (is_blocking == MPID_NOTBLOCKING) {
    ...
    PIbrecv( MPID_PT2PT_TAG, &pkt, sizeof(MPID_PKT_T), MSG_OTHER );
    }
}       /* #PVM3_END# */
#endif
...
if (MPID_PKT_IS_MSG(MPID_PKT.head.mode)) {
	...
	printf( 
       "[%d]R received message for tag = %d, source = %d, ctx = %d, mode = ", 
	       PImytid, MPID_PKT.head.tag, from, MPID_PKT.head.context_id );
...
if (DebugFlag) {
	...
	switch (MPID_PKT.head.mode) {
	    case MPID_PKT_SHORT_READY:
	    case MPID_PKT_SHORT:
	    err = MPID_CH_Copy_body_short( dmpi_recv_handle, &MPID_PKT, 
	...
	case MPID_PKT_LONG_READY:
	case MPID_PKT_LONG:
	    err = MPID_CH_Copy_body_long( dmpi_recv_handle, &MPID_PKT, from );
	    break;
	case MPID_PKT_LONG_SYNC:
	    err = MPID_CH_Copy_body_sync_long( dmpi_recv_handle, &MPID_PKT, 
					       from );
	    break;
#endif
	case MPID_PKT_SHORT_SYNC:
	    err = MPID_CH_Copy_body_sync_short( dmpi_recv_handle, &MPID_PKT, 
					        from );
	    break;
	    }
	}
    else {
	MPID_CH_Copy_body_unex( dmpi_recv_handle, &MPID_PKT, from );
	}
    }
else {
    switch (MPID_PKT.head.mode) {
	...
	MPID_CH_Do_Request( MPID_PKT.sendok_pkt.use_tag, from, 
 ...
 */
int MPID_CH_complete_recv( dmpi_recv_handle ) 
MPIR_RHANDLE *dmpi_recv_handle;
{
...
/*
   ...
   queue.
 */
int MPID_CH_blocking_recv( dmpi_recv_handle ) 
MPIR_RHANDLE *dmpi_recv_handle;
{
MPID_PKT_LALLOC
...
while (!dmpi_save_recv_handle->completed) {
	...
	if (DebugFlag) {
      ...
      "[%d]R received message for tag = %d, source = %d, ctx = %d (%s:%d)\n", 
		   PImytid, ptag, from, pcid, __FILE__, __LINE__ );
	    fflush( stdout );
	    }
#endif                  /* #DEBUG_END# */
	if (pcid == context_id        && 
...
#endif                  /* #DEBUG_END# */
	if (is_posted) {
	    err = MPID_CH_Copy_body( dmpi_recv_handle, &MPID_PKT, from );
#ifdef MPID_USE_RNDV
...
/* Respond to a request to send a message when the message is found to
   be posted */
int MPID_CH_Ack_Request( mpid_recv_handle, from, send_id )
MPID_RHANDLE *mpid_recv_handle;
int          from;
MPID_Aint    send_id;
{
...
PInrecv( tag, mpid_recv_handle->start, mpid_recv_handle->bytes_as_contig, 
	 MSG_OTHER, mpid_recv_handle->rid );
#endif
...
PIbsend( MPID_PT2PT_TAG, &pkt, sizeof(MPID_PKT_OK_TO_SEND_T), from, 
	 MSG_OTHER );
return MPI_SUCCESS;
}
...
PIwrecv( 0, 0, 0, 0, mpid_recv_handle->rid );
mpid_recv_handle->rid = 0;
}

/* Fullfill a request for a message */
int MPID_CH_Do_Request( use_tag, from, send_id )
int          use_tag, from;
MPID_Aint    send_id;
{
...
PInsendrr( use_tag, dmpi_send_handle->dev_shandle.start,
	   dmpi_send_handle->dev_shandle.bytes_as_contig, from, MSG_OTHER, 
