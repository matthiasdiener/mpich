/*
 *  $Id: chrecv.c,v 1.42 1995/07/31 14:47:08 gropp Exp $
 *
...
#ifndef lint
static char vcid[] = "$Id: chrecv.c,v 1.42 1995/07/31 14:47:08 gropp Exp $";
#endif /* lint */
...
/* them.                                                                   */
/***************************************************************************/
static int MPID_CH_Copy_body_short( dmpi_recv_handle, pkt, pktbuf )
...
static int MPID_CH_Copy_body_sync_short( dmpi_recv_handle, pkt, from )
MPIR_RHANDLE *dmpi_recv_handle;
MPID_PKT_T   *pkt;
int          from;
{
...
/* 
    ...
    sender for the data...
 */
static int MPID_CH_Copy_body_long( dmpi_recv_handle, pkt, from )
MPIR_RHANDLE *dmpi_recv_handle;
MPID_PKT_T   *pkt;
int          from;
{
...
int MPID_CH_Cmpl_recv_nb( dmpi_recv_handle )
MPIR_RHANDLE *dmpi_recv_handle;
{
}

int MPID_CH_Cmpl_recv_sync( dmpi_recv_handle )
MPIR_RHANDLE *dmpi_recv_handle;
{
/* ??? */
...
static int MPID_CH_Copy_body_sync_long( dmpi_recv_handle, pkt, from )
MPIR_RHANDLE *dmpi_recv_handle;
MPID_PKT_T   *pkt;
int          from;
{
...
MPID_CH_complete_recv( dmpi_unexpected );
 ...
 */
int MPID_CH_post_recv( dmpi_recv_handle ) 
MPIR_RHANDLE *dmpi_recv_handle;
{
...
#ifdef MPID_USE_RNDV
    return MPID_CH_Process_unexpected_rndv( dmpi_recv_handle, 
					    dmpi_unexpected );
#elif defined(MPID_USE_GET)
    return MPID_CH_Process_unexpected_get( dmpi_recv_handle, 
					    dmpi_unexpected );
#else
...
switch (pkt->head.mode) {
    ...
    MPID_CH_Ack_Request( dmpi_recv_handle, from, pkt->request_pkt.send_id,
			 pkt->head.len );
    ...
    MPID_CH_Do_get( dmpi_recv_handle, from, (MPID_PKT_GET_T *)pkt );
    ...
    case MPID_PKT_LONG:
#ifdef MPID_USE_RNDV
    err = MPID_CH_Copy_body_long_rndv( dmpi_recv_handle, pkt, from );
#else
    err = MPID_CH_Copy_body_long( dmpi_recv_handle, pkt, from );
#endif
    DMPI_mark_recv_completed(dmpi_recv_handle);
...
switch (pkt->head.mode) {
	...
	MPID_CH_Do_get_to_mem( address, from, (MPID_PKT_GET_T *)pkt );
...
/* Look for XDR bit */
	...
	switch (MPID_PKT_RECV_GET(pkt,head.mode)) {
	    case MPID_PKT_SHORT_READY:
	    case MPID_PKT_SHORT:
	    err = MPID_CH_Copy_body_short( dmpi_recv_handle, 
	...
	case MPID_PKT_REQUEST_SEND_READY:
	    ...
	    MPID_CH_Ack_Request( dmpi_recv_handle, from, 
	...
	case MPID_PKT_LONG:
#ifdef MPID_USE_RNDV
	    err = MPID_CH_Copy_body_long_rndv( dmpi_recv_handle, 
					       MPID_PKT_RECV_ADDR(pkt), from );
#else
	    err = MPID_CH_Copy_body_long( dmpi_recv_handle, 
					  MPID_PKT_RECV_ADDR(pkt), from );
#endif
	    break;
	case MPID_PKT_LONG_SYNC:
	    err = MPID_CH_Copy_body_sync_long( dmpi_recv_handle, 
					       MPID_PKT_RECV_ADDR(pkt), 
					       from );
	    break;
#endif
	case MPID_PKT_SHORT_SYNC:
	    err = MPID_CH_Copy_body_sync_short( dmpi_recv_handle, 
					        MPID_PKT_RECV_ADDR(pkt), 
					        from );
	    break;
#ifdef MPID_USE_GET
        case MPID_PKT_DO_GET:
	    MPID_CH_Do_get( dmpi_recv_handle, from, 
	...
	MPID_CH_Copy_body_unex( dmpi_recv_handle, MPID_PKT_RECV_ADDR(pkt), 
			        from );
	}
    }
else {
	...
	MPID_CH_Do_Request( MPID_PKT_RECV_GET(pkt,sendok_pkt.recv_handle), 
...
#ifdef MPID_USE_GET
        case MPID_PKT_DONE_GET:
        MPID_CH_Done_get( MPID_PKT_RECV_ADDR(pkt), from );
        break;
	case MPID_PKT_CONT_GET:
	MPID_CH_Cont_get( MPID_PKT_RECV_ADDR(pkt), from );
	break;
#endif
...
switch (dmpi_recv_handle->completer) {
    ...
    MPID_CH_Cmpl_recv_rndv( dmpi_recv_handle );
    break;
#endif
#ifndef MPID_USE_RNDV
    case MPID_CMPL_RECV_NB:
    MPID_CH_Cmpl_recv_nb( dmpi_recv_handle );
    break;
#endif
#ifdef MPID_USE_GET    
...
int MPID_CH_Test_recv_push( dmpi_recv_handle )
MPIR_RHANDLE *dmpi_recv_handle;
{
#ifdef MPID_USE_RNDV
if (dmpi_recv_handle->completer == MPID_CMPL_RECV_RNDV) {
    return MPID_CH_Test_recv_rndv( dmpi_recv_handle );
    }
#endif
...
#endif                  /* #DEBUG_END# */
	if (is_posted) {
	    err = MPID_CH_Copy_body( dmpi_recv_handle, 
				     MPID_PKT_RECV_ADDR(pkt), from );
