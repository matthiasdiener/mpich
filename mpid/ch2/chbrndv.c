...
void         *in_runex;
{
    ...
    MPID_CH_Rndvb_ok_to_send( runex->send_id, rtag, runex->from );
...
void         *in_pkt;
{
    ...
    MPID_DO_HETERO(rhandle->msgrep = (MPID_Msgrep_t)pkt->msgrep );
    /* Need to set the push etc routine to complete this transfer */
    rhandle->push = MPID_CH_Rndvb_unxrecv_start_self;
    return 0;
}
 ...
 */
MPID_Protocol *MPID_CH_Rndvb_setup()
{
    MPID_Protocol *p;

    p = (MPID_Protocol *) MALLOC( sizeof(MPID_Protocol) );
    if (!p) return 0;
    p->send	   = MPID_CH_Rndvb_send;
    p->recv	   = 0;
    p->isend	   = MPID_CH_Rndvb_isend;
    p->wait_send   = 0;
    p->push_send   = 0;
    p->cancel_send = 0;
    p->irecv	   = MPID_CH_Rndvb_irecv;
    p->wait_recv   = 0;
    p->push_recv   = 0;
    p->cancel_recv = 0;
    p->do_ack      = MPID_CH_Rndvb_ack;
    p->unex        = MPID_CH_Rndvb_save;
    p->delete      = MPID_CH_Rndvb_delete;
