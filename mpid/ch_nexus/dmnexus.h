...
#define PI_NO_NRECV
/*
 *  $Id: dmch.h,v 1.38 1995/08/11 00:21:51 gropp Exp gropp $
 *
...
/* Note that IBM MPL requires a struct for ASYNCRecvId_t in order to
   provide correctness:
if defined(eui) 
typedef struct { int rid, tag, from; } ASYNCRecvId_t;
 */
typedef long ASYNCRecvId_t;
typedef long ASYNCSendId_t;
#endif 
...
#define MPID_Post_send_sync(ctx,dmpi_send_handle) \
    MPID_NEXUS_post_send_sync(dmpi_send_handle) 
...
#define MPID_Blocking_send(ctx, dmpi_send_handle) \
    MPID_NEXUS_Blocking_send(dmpi_send_handle)
...
#define MPID_Test_send( ctx, dmpi_send_handle ) \
    ((dmpi_send_handle)->completer == 0 ? \
     1 : MPID_NEXUS_Test_send( dmpi_send_handle ))

#define MPID_Post_recv(ctx,dmpi_recv_handle ) \
    MPID_NEXUS_post_recv(dmpi_recv_handle ) 
#define MPID_Blocking_recv(ctx,dmpi_recv_handle ) \
    MPID_NEXUS_blocking_recv(dmpi_recv_handle) 
...
#define MPID_Test_recv( ctx, dmpi_recv_handle ) \
    ((dmpi_recv_handle)->completer == 0 ? \
        1 : MPID_NEXUS_Test_recv_push( dmpi_recv_handle )) 
...
#define MPID_Test_request( ctx, request ) \
    ( (request)->chandle.handle_type == MPIR_SEND ? \
        MPID_NEXUS_Test_send(&(request)->shandle) : \
        MPID_NEXUS_Test_recv_push(&(request)->rhandle))
...
#define MPID_Check_device( ctx,blocking ) \
    MPID_NEXUS_check_device( blocking )

#define MPID_Iprobe( ctx, tag, source, context_id, flag, status ) \
    MPID_NEXUS_Iprobe( tag, source, context_id, flag, status ) 
#define MPID_Probe( ctx, tag, source, context_id, status ) \
    MPID_NEXUS_Probe( tag, source, context_id, status ) 

#define MPID_NODE_NAME( ctx, name, len ) \
    MPID_NEXUS_Node_name( name, len )
#define MPID_Version_name( ctx, name ) MPID_NEXUS_Version_name( name )
#ifndef MPID_WTIME
#define MPID_WTIME(ctx)         MPID_NEXUS_Wtime()
#endif
#define MPID_WTICK(ctx)         MPID_NEXUS_Wtick()
#define MPID_INIT(argc,argv) MPID_NEXUS_Init( argc, argv )
#define MPID_END(ctx)           MPID_NEXUS_End()

#define MPID_ABORT( ctx, errorcode ) MPID_NEXUS_Abort( errorcode );

#define MPID_CANCEL( ctx, r ) MPID_NEXUS_Cancel( r )

#define MPID_Myrank( ctx, rank ) MPID_NEXUS_Myrank( rank )
#define MPID_Mysize( ctx, size ) MPID_NEXUS_Mysize( size )
...
#define MPID_Comm_init(ctx,comm,newcomm) MPID_NEXUS_Comm_init(comm,newcomm)
#define MPID_Comm_free(ctx,comm) MPID_NEXUS_Comm_free(comm)
#define MPID_Barrier(ctx,comm) MPID_NEXUS_Barrier(comm)
...
#define MPID_Reduce_sum_int(ctx,send,recv,comm) \
      MPID_NEXUS_Reduce_sum_int(send,recv,comm)
#define MPID_Reduce_sum_double(ctx,send,recv,comm) \
      MPID_NEXUS_Reduce_sum_double(send,recv,comm)
 ...
 */
#define MPID_NEXUSK_MSGLEN(dmpi_recv_handle,msglen,err) \
...
#define MPID_PKT_PACK(pkt,size,dest) MPID_NEXUS_Pkt_pack(pkt,size,dest)
#define MPID_PKT_UNPACK(pkt,size,src) MPID_NEXUS_Pkt_unpack(pkt,size,src)
#else
...
#define MPID_DRAIN_INCOMING \
    while (MPID_NEXUS_check_incoming( MPID_NOTBLOCKING ) != -1) ;
...
#define MPID_Dest_byte_order(dest) MPID_NEXUS_Dest_byte_order(dest)
#endif 
