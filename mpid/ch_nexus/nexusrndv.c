/*
 *  $Id: chrndv.c,v 1.2 1995/08/11 00:21:37 gropp Exp $
 *
...
#ifndef lint
static char vcid[] = "$Id: chrndv.c,v 1.2 1995/08/11 00:21:37 gropp Exp $";
#endif /* lint */
...
/* 
    ...
    sender for the data. ???
 */
int MPID_NEXUS_Copy_body_long_rndv( dmpi_recv_handle, pkt, from )
 ...
 */
int MPID_NEXUS_Process_unexpected_rndv( dmpi_recv_handle, dmpi_unexpected )
...
if (mpid_recv_handle_unex->send_id) {
    MPID_NEXUS_Ack_Request( dmpi_recv_handle, mpid_recv_handle_unex->from,
 ...
 */
int MPID_NEXUS_Save_unex_rndv( dmpi_recv_handle, pkt, from )
MPIR_RHANDLE *dmpi_recv_handle;
MPID_PKT_T   *pkt;
int          from;
{
 ...
 */
int MPID_NEXUS_Complete_recv_rndv( dmpi_recv_handle ) 
MPIR_RHANDLE *dmpi_recv_handle;
{
...
if (!MPID_Test_handle(dmpi_recv_handle) && dmpi_recv_handle->dev_rhandle.rid) {
    MPID_NEXUS_Complete_Rndv( &dmpi_recv_handle->dev_rhandle );
    DMPI_mark_recv_completed(dmpi_recv_handle);
    }
...
/* Respond to a request to send a message when the message is found to
   be posted */
int MPID_NEXUS_Ack_Request( dmpi_recv_handle, from, send_id, msglen )
MPIR_RHANDLE *dmpi_recv_handle;
int          from;
MPID_Aint    send_id;
int          msglen;
...
MPID_NEXUS_Complete_Rndv( mpid_recv_handle )
MPID_RHANDLE *mpid_recv_handle;
{
...
/* This is a test for received.  It must look to see if the transaction 
   has completed. */
int MPID_NEXUS_Test_recv_rndv( dmpi_recv_handle )
MPIR_RHANDLE *dmpi_recv_handle;
{
...
int MPID_NEXUS_Do_Request( recv_handle, from, send_id )
MPID_RNDV_T  recv_handle;
int          from;
MPID_Aint    send_id;
{
 ...
 */
int MPID_NEXUS_Test_send_rndv( dmpi_send_handle )
MPIR_SHANDLE *dmpi_send_handle;
{
...
int MPID_NEXUS_post_send_long_rndv( dmpi_send_handle, mpid_send_handle, len ) 
MPIR_SHANDLE *dmpi_send_handle;
MPID_SHANDLE *mpid_send_handle;
int len;
{
...
/* This test lets us 'complete' a rendevous send when there is no nonblocking
   send. */
if (mpid_send_handle->sid) {
    MPID_NEXUS_Test_send( dmpi_send_handle );
    }
#endif
...
int MPID_NEXUS_Cmpl_recv_rndv( dmpi_recv_handle )
MPIR_RHANDLE *dmpi_recv_handle;
{
 /*  && !defined(PI_NO_NRECV) */
