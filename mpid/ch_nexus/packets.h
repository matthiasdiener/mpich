...
#ifndef SPOOF_COPYING
#define MPID_PKT_MAX_DATA_SIZE 16384
#else
#define MPID_PKT_MAX_DATA_SIZE 2000000
#endif
#endif
#endif
...
#ifdef MPID_HAS_HETERO
#define DECL_SYNC_ID    int       sync_id
#else
...
/* 
 * For experimental purposes, I am making all message types be the same
 * size, but will use them as different packets.
 */
typedef struct {
    MPID_PKT_BASIC
    } MPID_PKT_HEAD_T;
...
#define MPID_PKT_WAIT() \
    {PIwrecv(0,0,0,0,&pktid); from = PIfrom();}
...
#define MPID_PKT_RECV_ADDR(pkt) (pkt)
#define MPID_PKT_RECV_FREE(pkt) \
    {if (pkt) PIFreeRecvBuf(pkt);}
#define MPID_PKT_RECV_CLR(pkt) pkt = 0;

#define MPID_PKT_INIT() \
    PINewRecvBuf(pkt,sizeof(MPID_PKT_T),MPID_PKT_T);
#define MPID_PKT_CHECK()  \
