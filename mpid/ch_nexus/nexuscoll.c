...
void *MPID_NEXUS_Init_barrier();
double MPID_NEXUS_Barrier_sum_double();
...
int MPID_NEXUS_Comm_init( comm, newcomm )
MPI_Comm comm, newcomm;
{
newcomm->ADIBarrier = MPID_NEXUS_Init_barrier( newcomm->local_group->np, 1 );
...
MPID_NEXUS_Setup_barrier( newcomm->ADIBarrier, newcomm->local_group->local_rank );

return MPI_SUCCESS;
}    
int MPID_NEXUS_Comm_free( comm )
MPI_Comm comm;
{
if (comm->ADIBarrier)
    MPID_NEXUS_Free_barrier( comm->ADIBarrier, 
			  comm->local_group->local_rank == 0 );

return MPI_SUCCESS;
}

void MPID_NEXUS_Barrier( comm ) 
MPI_Comm comm;
{
MPID_NEXUS_Wait_barrier( comm->ADIBarrier );
}

void MPID_NEXUS_Reduce_sum_int( sendbuf, recvbuf, comm )
int *sendbuf, *recvbuf;
MPI_Comm comm;
{
}

void MPID_NEXUS_Reduce_sum_double( sendbuf, recvbuf, comm )
double *sendbuf, *recvbuf;
MPI_Comm comm;
{
*recvbuf = MPID_NEXUS_Barrier_sum_double( comm->ADIReduce, sendbuf );
}
