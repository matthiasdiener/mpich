...
MPID_N3_Setup_barrier(bar,mypidf)
MPID_Fastbar *bar;
int *mypidf;
{
    int mypid,np;
...
double MPID_N3_Barrier_sum_double(bar,x)
MPID_Fastbar *bar;
double *x;
{
    register int oldphase;
...
double MPID_N3_Barrier_max_double(bar,x)
MPID_Fastbar *bar;
double *x;
{
    register int oldphase;
...
double MPID_N3_Barrier_min_double(bar,x)
MPID_Fastbar *bar;
double *x;
{
    register int oldphase;
...
char **argv;
{
    ...
    bar = MPID_N3_init_barrier(&n);

    p4_create_procgroup();
    myid = p4_get_my_id();
    x = (double) myid;
    /* Initialize the barriers */
    MPID_N3_Setup_barrier(bar,&myid);
    ...
    starttime = p4_ustimer();
    for (i = 0; i < 1000; i++)
      ysum = MPID_N3_Barrier_sum_double(bar,&x);
    endtime = p4_ustimer();
