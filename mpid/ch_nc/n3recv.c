...
/* buffer.                                                                 */
/***************************************************************************/
int MPID_N3_Copy_body_short( dmpi_recv_handle, pkt, pktbuf )
...
int MPID_N3_Copy_body_sync_short( dmpi_recv_handle, pkt, from )
MPIR_RHANDLE *dmpi_recv_handle;
MPID_PKT_T   *pkt;
int          from;
{
...
/* 
    ...
    sender for the data...
 */
int MPID_N3_Copy_body_long( dmpi_recv_handle, pkt, from )
MPIR_RHANDLE *dmpi_recv_handle;
MPID_PKT_T   *pkt;
int          from;
{
...
__N3FROM=-1;__N3TYPE=MPID_PT2PT2_TAG(from);nread(mpid_recv_handle->start,msglen,&__N3FROM,&__N3TYPE,&__N3LEN);
DMPI_mark_recv_completed(dmpi_recv_handle);

return err;
}

int MPID_N3_Copy_body_sync_long( dmpi_recv_handle, pkt, from )
MPIR_RHANDLE *dmpi_recv_handle;
MPID_PKT_T   *pkt;
int          from;
{
...
if (mpid_recv_handle_unex->send_id) {
    MPID_N3_Ack_Request( mpid_recv_handle, mpid_recv_handle_unex->from,
...
/*
   ...
   processing.
 */
int MPID_N3_post_recv( dmpi_recv_handle, is_available ) 
MPIR_RHANDLE *dmpi_recv_handle;
int          *is_available;
{
...
while (MPID_N3_check_incoming( MPID_NOTBLOCKING ) != -1) ;
...
switch (pkt->head.mode) {
    ...
    /* Send back an OK to send, with a tag value and 
       a posted recv */
    MPID_N3_Ack_Request( &dmpi_recv_handle->dev_rhandle, from, 
...
switch (pkt->head.mode) {
	...
	MPIDGETMEM(msglen);
	MPID_KEEP_STAT(n_long++;)
	__N3FROM=-1;__N3TYPE=MPID_PT2PT2_TAG(from);nread(address,msglen,&__N3FROM,&__N3TYPE,&__N3LEN);
	break;
#endif
    }
...
if (is_blocking == MPID_NOTBLOCKING) {
    ...
    __N3FROM=-1;__N3TYPE=MPID_PT2PT_TAG;nread(&pkt,sizeof(MPID_PKT_T),&__N3FROM,&__N3TYPE,&__N3LEN);
    }
}       /* #PVM3_END# */
#endif
...
if (DebugFlag) {
	...
	switch (MPID_PKT.head.mode) {
	    case MPID_PKT_SHORT_READY:
	    case MPID_PKT_SHORT:
	    err = MPID_N3_Copy_body_short( dmpi_recv_handle, &MPID_PKT, 
	...
	case MPID_PKT_LONG_READY:
	case MPID_PKT_LONG:
	    err = MPID_N3_Copy_body_long( dmpi_recv_handle, &MPID_PKT, from );
	    break;
	case MPID_PKT_LONG_SYNC:
	    err = MPID_N3_Copy_body_sync_long( dmpi_recv_handle, &MPID_PKT, 
					       from );
	    break;
#endif
	case MPID_PKT_SHORT_SYNC:
	    err = MPID_N3_Copy_body_sync_short( dmpi_recv_handle, &MPID_PKT, 
					        from );
	    break;
	    }
	}
    else {
	MPID_N3_Copy_body_unex( dmpi_recv_handle, &MPID_PKT, from );
	}
    }
else {
    switch (MPID_PKT.head.mode) {
	...
	MPID_N3_Do_Request( MPID_PKT.sendok_pkt.use_tag, from, 
...
if (!dmpi_recv_handle->completed && dmpi_recv_handle->dev_rhandle.rid) {
    MPID_N3_Complete_Rndv( &dmpi_recv_handle->dev_rhandle );
...
#endif                  /* #DEBUG_END# */
	if (is_posted) {
	    err = MPID_N3_Copy_body( dmpi_recv_handle, &MPID_PKT, from );
#ifdef MPID_USE_RNDV
...
/* Respond to a request to send a message when the message is found to
   be posted */
int MPID_N3_Ack_Request( mpid_recv_handle, from, send_id )
MPID_RHANDLE *mpid_recv_handle;
int          from;
MPID_Aint    send_id;
{
...
_nwrite(&pkt,sizeof(MPID_PKT_OK_TO_SEND_T),from,MPID_PT2PT_TAG,&__N3FLAG);
return MPI_SUCCESS;
}

MPID_N3_Complete_Rndv( mpid_recv_handle )
MPID_RHANDLE *mpid_recv_handle;
{
if (--TagsInUse == 0) CurTag = 1;
__N3FROM=-1;__N3TYPE=0;nread(0,0,&__N3FROM,&__N3TYPE,&__N3LEN);
mpid_recv_handle->rid = 0;
}

/* Fullfill a request for a message */
int MPID_N3_Do_Request( use_tag, from, send_id )
int          use_tag, from;
MPID_Aint    send_id;
{
...
mpid_send_handle->sid =0;_nwrite(dmpi_send_handle->dev_shandle.start,dmpi_send_handle->dev_shandle.bytes_as_contig,from,use_tag,&__N3FLAG);
return MPI_SUCCESS;
}
#endif
