...
MPID_PKT_SEND_SET(pkt,sync_id,
		  MPID_N3_Get_Sync_Id( dmpi_send_handle, mpid_send_handle ));
...
if (actual_len > MPID_PKT_DATA_SIZE) 
    ...
    rc = MPID_N3_post_send_long_rndv( dmpi_send_handle, mpid_send_handle, 
				      actual_len );
#else
    rc = MPID_N3_post_send_long( dmpi_send_handle, mpid_send_handle, 
				   actual_len );
#endif
else
...
if (actual_len > MPID_PKT_DATA_SIZE) 
    ...
    rc = MPID_N3_post_send_sync_long_get( dmpi_send_handle, mpid_send_handle, 
					  actual_len );
#else
    rc = MPID_N3_post_send_sync_long( dmpi_send_handle, mpid_send_handle, 
				      actual_len );
#endif
else
...
switch (dmpi_send_handle->completer) {
    ...
    case MPID_CMPL_SEND_SYNC:
	 ...
	 MPID_N3_Cmpl_send_sync( dmpi_send_handle );
	 break;
    default:
