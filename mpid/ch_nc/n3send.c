...
#ifdef MPID_DEBUG_ALL   /* #DEBUG_START# */
    if (DebugFlag) {
	MPID_N3_Print_pkt_data( "S Getting data from mpid->start", 
			        pkt.buffer, len );
	}
#endif                  /* #DEBUG_END# */
    }
...
#endif
_nwrite(&pkt,len + sizeof(MPID_PKT_HEAD_T),dest,MPID_PT2PT_TAG,&__N3FLAG);
...
/* Long message */
int MPID_N3_post_send_long( dmpi_send_handle, mpid_send_handle, len ) 
MPIR_SHANDLE *dmpi_send_handle;
MPID_SHANDLE *mpid_send_handle;
int len;
{
...
if (mpid_send_handle->is_non_blocking) 
    while (MPID_N3_check_incoming( MPID_NOTBLOCKING ) != -1) ;
#endif
_nwrite(&pkt,sizeof(MPID_PKT_LONG_T),dest,MPID_PT2PT_TAG,&__N3FLAG);
...
#ifndef PI_NO_NSEND
if (mpid_send_handle->is_non_blocking) {
    mpid_send_handle->sid =0;_nwrite(address,len,dest,MPID_PT2PT2_TAG(__MYPROCID),&__N3FLAG);
    }
else 
#endif
    {
    mpid_send_handle->sid = 0;
    _nwrite(address,len,dest,MPID_PT2PT2_TAG(__MYPROCID),&__N3FLAG);
    DMPI_mark_send_completed( dmpi_send_handle );
    }
#endif
return MPI_SUCCESS;
...
if (dmpi_send_handle->msgrep == MPIR_MSGREP_XDR) 
    pkt.has_xdr = MPID_MODE_XDR;
#endif                         /* #HETERO_END# */
pkt.sync_id      = MPID_N3_Get_Sync_Id( dmpi_send_handle, mpid_send_handle );
...
/* In case the message is marked as non-blocking, indicate that we don't
   need to wait on it */
mpid_send_handle->sid = 0;
_nwrite(&pkt,len + (sizeof(MPID_PKT_SHORT_SYNC_T)-MPID_PKT_MAX_DATA_SIZE),dest,MPID_PT2PT_TAG,&__N3FLAG);
...
/* Long message */
int MPID_N3_post_send_sync_long( dmpi_send_handle, mpid_send_handle, len ) 
MPIR_SHANDLE *dmpi_send_handle;
MPID_SHANDLE *mpid_send_handle;
int len;
{
...
#endif                  /* #DEBUG_END# */
/* Send as packet only */
_nwrite(&pkt,sizeof(MPID_PKT_LONG_SYNC_T),dest,MPID_PT2PT_TAG,&__N3FLAG);
 ...
 */
int MPID_N3_post_send( dmpi_send_handle ) 
MPIR_SHANDLE *dmpi_send_handle;
{
MPID_SHANDLE *mpid_send_handle;
int         actual_len;
...
if (actual_len > MPID_PKT_DATA_SIZE) 
    return MPID_N3_post_send_long( dmpi_send_handle, mpid_send_handle, 
				   actual_len );
else
    return MPID_N3_post_send_short( dmpi_send_handle, mpid_send_handle, 
				    actual_len );
}

int MPID_N3_post_send_sync( dmpi_send_handle ) 
MPIR_SHANDLE *dmpi_send_handle;
{
MPID_SHANDLE *mpid_send_handle;
int         actual_len;
...
if (actual_len > MPID_PKT_DATA_SIZE) 
    ...
    return MPID_N3_post_send_long( dmpi_send_handle, mpid_send_handle, 
				   actual_len );
#else
    return MPID_N3_post_send_sync_long( dmpi_send_handle, mpid_send_handle, 
				        actual_len );
#endif
else
    return MPID_N3_post_send_sync_short( dmpi_send_handle, mpid_send_handle, 
					 actual_len );
}
 ...
 */
int MPID_N3_isend_wait( dmpi_send_handle )
MPIR_SHANDLE *dmpi_send_handle;
{
MPID_SHANDLE *mpid_send_handle;
 ...
 */
int MPID_N3_complete_send( dmpi_send_handle ) 
MPIR_SHANDLE *dmpi_send_handle;
{
...
while (!dmpi_send_handle->completed) {
    /* This waits for the completion of a synchronous send, since at
       this point, we've finished waiting for the =0;_nwrite(,,,,&__N3FLAG) to complete */
    (void)MPID_N3_check_incoming( MPID_BLOCKING );
    }
#ifdef MPID_DEBUG_ALL   /* #DEBUG_START# */
if (DebugFlag) {
