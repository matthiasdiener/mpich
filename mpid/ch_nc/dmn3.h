...
#include <stdio.h>
extern int __NUMNODES,__MYPROCID,__N3LEN,__N3FROM,__N3TYPE,__N3FLAG;
/* #MEIKO DECLARATION# */
#else
/* 
...
#define MPID_Post_send_sync(ctx,dmpi_send_handle) \
    MPID_N3_post_send_sync(dmpi_send_handle) 
...
#define MPID_Blocking_send(ctx, dmpi_send_handle) \
    MPID_N3_Blocking_send(dmpi_send_handle)
#else
#define MPID_Blocking_send(ctx, dmpi_send_handle) \
{int _err;\
...
#define MPID_Test_send( ctx, dmpi_send_handle ) \
    ((dmpi_send_handle)->completer == 0 ? \
     1 : MPID_N3_Test_send( dmpi_send_handle ))

#define MPID_Post_recv(ctx,dmpi_recv_handle ) \
    MPID_N3_post_recv(dmpi_recv_handle ) 
#define MPID_Blocking_recv(ctx,dmpi_recv_handle ) \
    MPID_N3_blocking_recv(dmpi_recv_handle) 
...
#define MPID_Test_recv( ctx, dmpi_recv_handle ) \
    ((dmpi_recv_handle)->completer == 0 ? \
        1 : MPID_N3_Test_recv_push( dmpi_recv_handle )) 
...
#define MPID_Test_request( ctx, request ) \
    ( (request)->chandle.handle_type == MPIR_SEND ? \
        MPID_N3_Test_send(&(request)->shandle) : \
        MPID_N3_Test_recv_push(&(request)->rhandle))
...
#define MPID_Check_device( ctx,blocking ) \
    MPID_N3_check_device( blocking )

#define MPID_Iprobe( ctx, tag, source, context_id, flag, status ) \
    MPID_N3_Iprobe( tag, source, context_id, flag, status ) 
#define MPID_Probe( ctx, tag, source, context_id, status ) \
    MPID_N3_Probe( tag, source, context_id, status ) 

#define MPID_NODE_NAME( ctx, name, len ) \
    MPID_N3_Node_name( name, len )
#define MPID_Version_name( ctx, name ) MPID_N3_Version_name( name )
#ifndef MPID_WTIME
#define MPID_WTIME(ctx)         MPID_N3_Wtime()
#endif
#define MPID_WTICK(ctx)         MPID_N3_Wtick()
#define MPID_INIT(argc,argv) MPID_N3_Init( argc, argv )
#define MPID_END(ctx)           MPID_N3_End()

#define MPID_ABORT( ctx, errorcode ) MPID_N3_Abort( errorcode );

#define MPID_CANCEL( ctx, r ) MPID_N3_Cancel( r )

#define MPID_Myrank( ctx, rank ) MPID_N3_Myrank( rank )
#define MPID_Mysize( ctx, size ) MPID_N3_Mysize( size )
...
#define MPID_Comm_init(ctx,comm,newcomm) MPID_N3_Comm_init(comm,newcomm)
#define MPID_Comm_free(ctx,comm) MPID_N3_Comm_free(comm)
#define MPID_Barrier(ctx,comm) MPID_N3_Barrier(comm)
...
#define MPID_Reduce_sum_int(ctx,send,recv,comm) \
      MPID_N3_Reduce_sum_int(send,recv,comm)
#define MPID_Reduce_sum_double(ctx,send,recv,comm) \
      MPID_N3_Reduce_sum_double(send,recv,comm)
...
#define MPID_Comm_init(ctx,comm,newcomm) MPID_N3_Comm_msgrep( newcomm )
#else
...
/* Common macro for checking the actual length (msglen) against the
   ...
   situations.
 */
#define MPID_N3K_MSGLEN(dmpi_recv_handle,msglen,err) \
 ...
 */
#ifdef MPID_HAS_HETERO
#define MPID_PKT_PACK(pkt,size,dest) MPID_N3_Pkt_pack((MPID_PKT_T*)(pkt),size,dest)
#define MPID_PKT_UNPACK(pkt,size,src) MPID_N3_Pkt_unpack((MPID_PKT_T*)(pkt),size,src)
#else
...
#define MPID_Dest_byte_order(dest) MPID_N3_Dest_byte_order(dest)
#endif 
