...
MPID_Aint MPID_N3_Get_Sync_Id( dmpi_handle, mpid_handle )
...
if (!new) MPID_N3_Abort( MPI_ERR_EXHAUSTED );
new->id		      = CurId++;
...
int MPID_N3_Lookup_SyncAck( sync_id, dmpi_send_handle, mpid_send_handle )
int          sync_id;
MPIR_SHANDLE **dmpi_send_handle;
MPID_SHANDLE **mpid_send_handle;
{
...
#endif                  /* #DEBUG_END# */
MPID_N3_Lookup_SyncAck( sync_id, &dmpi_send_handle, &mpid_send_handle );
if (!dmpi_send_handle) {
...
#endif                  /* #DEBUG_END# */
_nwrite(&pkt,sizeof(MPID_PKT_SYNC_ACK_T),from,MPID_PT2PT_TAG,&__N3FLAG);
}
