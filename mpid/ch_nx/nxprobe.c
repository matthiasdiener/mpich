 ...
 */
int MPID_NX_Iprobe( tag, source, context_id, found, status )
...
if (!*found) {
    ...
    MPID_NX_check_incoming( MPID_NOTBLOCKING );
    DMPI_search_unexpected_queue( source, tag, context_id, 
				  found, 0, &dmpi_unexpected );
    }
if (*found) {
...
void MPID_NX_Probe( tag, source, context_id, status )
int tag, source, context_id;
MPI_Status *status;
{
int found;
while (1) {
    MPID_NX_Iprobe( tag, source, context_id, &found, status );
    if (found) break;
    /* Wait for a message */
    MPID_NX_check_incoming( MPID_BLOCKING );
    }
}
