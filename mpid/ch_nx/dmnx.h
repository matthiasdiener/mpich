...
#include <stdio.h>
extern int __NUMNODES, __MYPROCID;typedef void *ProcSet;extern double dclock();
/* #MEIKO DECLARATION# */
#else
/* 
...
#define MPID_Test_send( ctx, dmpi_send_handle ) \
    ((dmpi_send_handle)->completer == 0 ? \
     1 : MPID_NX_Test_send( dmpi_send_handle ))
...
#define MPID_Test_recv( ctx, dmpi_recv_handle ) \
    ((dmpi_recv_handle)->completer == 0 ? \
        1 : MPID_NX_Test_recv_push( dmpi_recv_handle )) 
...
#define MPID_Test_request( ctx, request ) \
    ( (request)->chandle.handle_type == MPIR_SEND ? \
        ...
        MPID_NX_Test_recv_push(&(request)->rhandle))
...
#define MPID_Barrier(ctx,comm) MPID_NX_Barrier(comm)
...
#define MPID_Reduce_sum_int(ctx,send,recv,comm) \
      MPID_NX_Reduce_sum_int(send,recv,comm)
#define MPID_Reduce_sum_double(ctx,send,recv,comm) \
      MPID_NX_Reduce_sum_double(send,recv,comm)
...
#define MPID_Comm_init(ctx,comm,newcomm) MPID_NX_Comm_msgrep( newcomm )
#else
...
/* Common macro for checking the actual length (msglen) against the
   ...
   situations.
 */
#define MPID_NXK_MSGLEN(dmpi_recv_handle,msglen,err) \
 ...
 */
#ifdef MPID_HAS_HETERO
#define MPID_PKT_PACK(pkt,size,dest) MPID_NX_Pkt_pack((MPID_PKT_T*)(pkt),size,dest)
#define MPID_PKT_UNPACK(pkt,size,src) MPID_NX_Pkt_unpack((MPID_PKT_T*)(pkt),size,src)
#else
...
#define MPID_Dest_byte_order(dest) MPID_NX_Dest_byte_order(dest)
#endif 
