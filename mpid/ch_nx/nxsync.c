...
MPID_Aint MPID_NX_Get_Sync_Id( dmpi_handle, mpid_handle )
...
if (!new) MPID_NX_Abort( MPI_ERR_EXHAUSTED );
new->id		      = CurId++;
...
int MPID_NX_Lookup_SyncAck( sync_id, dmpi_send_handle, mpid_send_handle )
int          sync_id;
MPIR_SHANDLE **dmpi_send_handle;
MPID_SHANDLE **mpid_send_handle;
{
...
#endif                  /* #DEBUG_END# */
MPID_NX_Lookup_SyncAck( sync_id, &dmpi_send_handle, &mpid_send_handle );
if (!dmpi_send_handle) {
...
#endif                  /* #DEBUG_END# */
_csend(MPID_PT2PT_TAG,&pkt,sizeof(MPID_PKT_SYNC_ACK_T),from,0);
}
