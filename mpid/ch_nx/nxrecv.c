...
/* 
  ...
  a MPID_NX_check_incoming routine that looks for headers.  Note that
...
/* them.                                                                   */
/***************************************************************************/
static int MPID_NX_Copy_body_short( dmpi_recv_handle, pkt, pktbuf )
...
static int MPID_NX_Copy_body_sync_short( dmpi_recv_handle, pkt, from )
MPIR_RHANDLE *dmpi_recv_handle;
MPID_PKT_T   *pkt;
int          from;
{
...
err = MPID_NX_Copy_body_short( dmpi_recv_handle, pkt, 
...
/* 
    ...
    sender for the data...
 */
static int MPID_NX_Copy_body_long( dmpi_recv_handle, pkt, from )
MPIR_RHANDLE *dmpi_recv_handle;
MPID_PKT_T   *pkt;
int          from;
{
...
int MPID_NX_Cmpl_recv_nb( dmpi_recv_handle )
MPIR_RHANDLE *dmpi_recv_handle;
{
return MPI_SUCCESS;
}  

int MPID_NX_Cmpl_recv_sync( dmpi_recv_handle )
MPIR_RHANDLE *dmpi_recv_handle;
{
/* ??? */
...
static int MPID_NX_Copy_body_sync_long( dmpi_recv_handle, pkt, from )
MPIR_RHANDLE *dmpi_recv_handle;
MPID_PKT_T   *pkt;
int          from;
{
...
err = MPID_NX_Copy_body_long( dmpi_recv_handle, pkt, from );
 ...
 */
int MPID_NX_Process_unexpected( dmpi_recv_handle, dmpi_unexpected )
...
MPID_NX_complete_recv( dmpi_unexpected );
 ...
 */
int MPID_NX_post_recv( dmpi_recv_handle ) 
MPIR_RHANDLE *dmpi_recv_handle;
{
...
if (!dmpi_recv_handle->dev_rhandle.is_non_blocking) {
    return MPID_NX_blocking_recv( dmpi_recv_handle );
    }
...
if (found) {
    ...
    return MPID_NX_Process_unexpected_get( dmpi_recv_handle, 
					    dmpi_unexpected );
#elif defined(MPID_USE_RNDV)                  /*#NONGET_START#*/
    return MPID_NX_Process_unexpected_rndv( dmpi_recv_handle, 
					    dmpi_unexpected );
#else
    return MPID_NX_Process_unexpected( dmpi_recv_handle, dmpi_unexpected );
                                              /*#NONGET_END#*/
#endif
    }
 ...
 */
int MPID_NX_Copy_body( dmpi_recv_handle, pkt, from )
MPIR_RHANDLE *dmpi_recv_handle;
MPID_PKT_T   *pkt;
int          from;
{
...
switch (pkt->head.mode) {
    ...
    err = MPID_NX_Copy_body_sync_short( dmpi_recv_handle, pkt, from );
    DMPI_mark_recv_completed(dmpi_recv_handle);
    break;
    ...
    MPID_NX_Ack_Request( dmpi_recv_handle, from, pkt->request_pkt.send_id,
			 pkt->head.len );
    ...
    MPID_NX_Do_get( dmpi_recv_handle, from, (MPID_PKT_GET_T *)pkt );
    ...
    case MPID_PKT_LONG:
#ifdef MPID_USE_RNDV                         
    err = MPID_NX_Copy_body_long_rndv( dmpi_recv_handle, pkt, from );
#else                                        
    err = MPID_NX_Copy_body_long( dmpi_recv_handle, pkt, from );
#endif
    DMPI_mark_recv_completed(dmpi_recv_handle);
    ...
    err = MPID_NX_Copy_body_sync_long( dmpi_recv_handle, pkt, from );
    DMPI_mark_recv_completed(dmpi_recv_handle);
    break;
                                             /*#NONGET_END#*/
#endif                                       /*#GET_START#*/
 ...
 */
int MPID_NX_Copy_body_unex( dmpi_recv_handle, pkt, from )
MPIR_RHANDLE *dmpi_recv_handle;
MPID_PKT_T   *pkt;
int          from;
{
...
switch (pkt->head.mode) {
	...
	MPID_NX_Do_get_to_mem( address, from, (MPID_PKT_GET_T *)pkt );
...
/* dispatches them.  There is another such look in MPID_NX_blocking_recv   */
 ...
 */    
int MPID_NX_check_incoming( is_blocking )
...
/* Is the message expected or not? 
   ...
   is unexpected (is_posted == 0) */
	...
	switch (MPID_PKT_RECV_GET(pkt,head.mode)) {
	    case MPID_PKT_SHORT_READY:
	    case MPID_PKT_SHORT:
	    err = MPID_NX_Copy_body_short( dmpi_recv_handle, 
	...
	case MPID_PKT_REQUEST_SEND_READY:
	    ...
	    MPID_NX_Ack_Request( dmpi_recv_handle, from, 
	...
	case MPID_PKT_LONG:
#ifdef MPID_USE_RNDV                   
	    err = MPID_NX_Copy_body_long_rndv( dmpi_recv_handle, 
					       MPID_PKT_RECV_ADDR(pkt), from );
#else                                  
	    err = MPID_NX_Copy_body_long( dmpi_recv_handle, 
					  MPID_PKT_RECV_ADDR(pkt), from );
#endif                                 

	    break;
	case MPID_PKT_LONG_SYNC:
	    err = MPID_NX_Copy_body_sync_long( dmpi_recv_handle, 
					       MPID_PKT_RECV_ADDR(pkt), 
					       from );
	    break;
	                                      /*#NONGET_END#*/
	case MPID_PKT_SHORT_SYNC:
	    err = MPID_NX_Copy_body_sync_short( dmpi_recv_handle, 
					        MPID_PKT_RECV_ADDR(pkt), 
					        from );
	    break;
#ifdef MPID_USE_GET                    /*#GET_START#*/
        case MPID_PKT_DO_GET:
	    MPID_NX_Do_get( dmpi_recv_handle, from, 
	...
	MPID_NX_Copy_body_unex( dmpi_recv_handle, MPID_PKT_RECV_ADDR(pkt), 
			        from );
	}
    }
else {
	...
	MPID_NX_Do_Request( MPID_PKT_RECV_GET(pkt,sendok_pkt.recv_handle), 
...
#ifdef MPID_USE_GET                            /*#GET_START#*/
        case MPID_PKT_DONE_GET:
        MPID_NX_Done_get( MPID_PKT_RECV_ADDR(pkt), from );
        break;
	case MPID_PKT_CONT_GET:
	MPID_NX_Cont_get( MPID_PKT_RECV_ADDR(pkt), from );
	break;
#endif                                         /*#GET_END#*/
 ...
 */
int MPID_NX_complete_recv( dmpi_recv_handle ) 
MPIR_RHANDLE *dmpi_recv_handle;
{
...
switch (dmpi_recv_handle->completer) {
    ...
    MPID_NX_Cmpl_recv_rndv( dmpi_recv_handle );
    break;
#endif                                              /*#NONGET_END#*/
#ifndef MPID_USE_RNDV                               /*#NONGET_START#*/
    case MPID_CMPL_RECV_NB:
    DEBUG_PRINT_MSG("Complete nonblocking")
    MPID_NX_Cmpl_recv_nb( dmpi_recv_handle );
    break;
#endif                                              /*#NONGET_END#*/
#ifdef MPID_USE_GET                                 /*#GET_START#*/
...
int MPID_NX_Test_recv_push( dmpi_recv_handle )
MPIR_RHANDLE *dmpi_recv_handle;
{
#ifdef MPID_USE_RNDV                              /*#NONGET_START#*/
if (dmpi_recv_handle->completer == MPID_CMPL_RECV_RNDV) {
    return MPID_NX_Test_recv_rndv( dmpi_recv_handle );
    }
#endif                                            /*#NONGET_END#*/
...
/*
   ...
   complete-recv).
 */
int MPID_NX_blocking_recv( dmpi_recv_handle ) 
MPIR_RHANDLE *dmpi_recv_handle;
{
MPID_PKT_RECV_DECL(MPID_PKT_T,pkt);
...
#endif                  /* #DEBUG_END# */
	if (is_posted) {
	    err = MPID_NX_Copy_body( dmpi_recv_handle, 
				     MPID_PKT_RECV_ADDR(pkt), from );
