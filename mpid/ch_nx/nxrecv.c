...
_crecv(MPID_PT2PT2_TAG(from),mpid_recv_handle->start,msglen);
DMPI_mark_recv_completed(dmpi_recv_handle);
...
if (mpid_recv_handle_unex->send_id) {
    MPID_NX_Ack_Request( mpid_recv_handle, mpid_recv_handle_unex->from,
...
switch (pkt->head.mode) {
    ...
    /* Send back an OK to send, with a tag value and 
       a posted recv */
    MPID_NX_Ack_Request( &dmpi_recv_handle->dev_rhandle, from, 
...
switch (pkt->head.mode) {
	...
	MPIDGETMEM(msglen);
	MPID_KEEP_STAT(n_long++;)
	_crecv(MPID_PT2PT2_TAG(from),address,msglen);
	break;
#endif
    }
...
if (DebugFlag) {
	...
	switch (MPID_PKT.head.mode) {
	    case MPID_PKT_SHORT_READY:
	    case MPID_PKT_SHORT:
	    err = MPID_NX_Copy_body_short( dmpi_recv_handle, &MPID_PKT, 
	...
	case MPID_PKT_LONG_READY:
	case MPID_PKT_LONG:
	    err = MPID_NX_Copy_body_long( dmpi_recv_handle, &MPID_PKT, from );
	    break;
	case MPID_PKT_LONG_SYNC:
	    err = MPID_NX_Copy_body_sync_long( dmpi_recv_handle, &MPID_PKT, 
					       from );
	    break;
#endif
	case MPID_PKT_SHORT_SYNC:
	    err = MPID_NX_Copy_body_sync_short( dmpi_recv_handle, &MPID_PKT, 
					        from );
	    break;
	    }
	}
    else {
	MPID_NX_Copy_body_unex( dmpi_recv_handle, &MPID_PKT, from );
	}
    }
else {
    switch (MPID_PKT.head.mode) {
	...
	MPID_NX_Do_Request( MPID_PKT.sendok_pkt.use_tag, from, 
...
if (!dmpi_recv_handle->completed && dmpi_recv_handle->dev_rhandle.rid) {
    MPID_NX_Complete_Rndv( &dmpi_recv_handle->dev_rhandle );
...
#endif                  /* #DEBUG_END# */
	if (is_posted) {
	    err = MPID_NX_Copy_body( dmpi_recv_handle, &MPID_PKT, from );
#ifdef MPID_USE_RNDV
...
/* Respond to a request to send a message when the message is found to
   be posted */
int MPID_NX_Ack_Request( mpid_recv_handle, from, send_id )
MPID_RHANDLE *mpid_recv_handle;
int          from;
MPID_Aint    send_id;
{
...
mpid_recv_handle->rid =_irecv(tag,mpid_recv_handle->start,mpid_recv_handle->bytes_as_contig);
#endif
...
_csend(MPID_PT2PT_TAG,&pkt,sizeof(MPID_PKT_OK_TO_SEND_T),from,0);
return MPI_SUCCESS;
}

MPID_NX_Complete_Rndv( mpid_recv_handle )
MPID_RHANDLE *mpid_recv_handle;
{
if (--TagsInUse == 0) CurTag = 1;
msgwait(mpid_recv_handle->rid );
mpid_recv_handle->rid = 0;
}

/* Fullfill a request for a message */
int MPID_NX_Do_Request( use_tag, from, send_id )
int          use_tag, from;
MPID_Aint    send_id;
{
...
mpid_send_handle->sid =_isend((use_tag)|0x40000000,dmpi_send_handle->dev_shandle.start,dmpi_send_handle->dev_shandle.bytes_as_contig,from,0);
return MPI_SUCCESS;
}
#endif
