...
#define MPID_Post_send_sync(ctx,dmpi_send_handle) \
    MPID_SHMEM_post_send_sync(dmpi_send_handle) 
...
#define MPID_Test_request( ctx, request ) \
    ( (request)->chandle.handle_type == MPIR_SEND ? \
        MPID_SHMEM_Test_send(&(request)->shandle) : \
        MPID_SHMEM_Test_recv_push(&(request)->rhandle))
...
#define MPID_Check_device( ctx,blocking ) \
    MPID_SHMEM_check_device( blocking )

#define MPID_Iprobe( ctx, tag, source, context_id, flag, status ) \
    MPID_SHMEM_Iprobe( tag, source, context_id, flag, status ) 
#define MPID_Probe( ctx, tag, source, context_id, status ) \
    MPID_SHMEM_Probe( tag, source, context_id, status ) 
...
#define MPID_ABORT( ctx, errorcode ) MPID_SHMEM_Abort( errorcode );

#define MPID_CANCEL( ctx, r ) MPID_SHMEM_Cancel( r )

#define MPID_Myrank( ctx, rank ) MPID_SHMEM_Myrank( rank )
#define MPID_Mysize( ctx, size ) MPID_SHMEM_Mysize( size )
...
#define MPID_Reduce_sum_int(ctx,send,recv,comm) \
      MPID_SHMEM_Reduce_sum_int(send,recv,comm)
#define MPID_Reduce_sum_double(ctx,send,recv,comm) \
      MPID_SHMEM_Reduce_sum_double(send,recv,comm)
...
#define MPID_Comm_init(ctx,comm,newcomm) MPID_SHMEM_Comm_msgrep( newcomm )
#else
 ...
 */
#ifdef MPID_HAS_HETERO
#define MPID_PKT_PACK(pkt,size,dest) MPID_SHMEM_Pkt_pack((MPID_PKT_T*)(pkt),size,dest)
#define MPID_PKT_UNPACK(pkt,size,src) MPID_SHMEM_Pkt_unpack((MPID_PKT_T*)(pkt),size,src)
#else
