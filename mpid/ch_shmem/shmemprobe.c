 ...
 */
int MPID_SHMEM_Iprobe( tag, source, context_id, found, status )
...
if (!*found) {
    ...
    MPID_SHMEM_check_incoming( MPID_NOTBLOCKING );
    DMPI_search_unexpected_queue( source, tag, context_id, 
				  found, 0, &dmpi_unexpected );
    }
if (*found) {
...
void MPID_SHMEM_Probe( tag, source, context_id, status )
int tag, source, context_id;
MPI_Status *status;
{
int found;
while (1) {
    MPID_SHMEM_Iprobe( tag, source, context_id, &found, status );
    if (found) break;
    /* Wait for a message */
    MPID_SHMEM_check_incoming( MPID_BLOCKING );
    }
}
