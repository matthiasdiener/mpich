...
/* Send a short (single packet message) 
   ...
   macros expect dest) 
*/
int MPID_SHMEM_send_short( buf, len, tag, context_id, lrank_sender, dest,
		        msgrep ) 
void *buf;
...
void MPID_SHMEM_Cmpl_send_sync( dmpi_send_handle )
MPIR_SHANDLE *dmpi_send_handle;
{
 ...
 ***************************************************************************/
int MPID_SHMEM_Post_send_local( dmpi_send_handle, mpid_send_handle, len )
MPIR_SHANDLE *dmpi_send_handle;
MPID_SHANDLE *mpid_send_handle;
int           len;
{
...
if (dest == MPID_MyWorldRank) {
    return MPID_SHMEM_Post_send_local( dmpi_send_handle, mpid_send_handle, 
				    actual_len );
    }
 }
#endif
 ...
 */
int MPID_SHMEM_Contig_blocking_send( buf, len, tag, context_id, lrank_sender, 
				  grank_dest, msgrep )
void *buf;
...
if (len <= MPID_PKT_DATA_SIZE) 
    rc = MPID_SHMEM_send_short( buf, len, tag, context_id, lrank_sender, 
			     grank_dest, msgrep );
else 
#ifdef MPID_USE_GET
    rc = MPID_SHMEM_send_long_get( buf, len, tag, context_id, lrank_sender, 
			        grank_dest, msgrep );
#ifdef FOO
#endif
#else
