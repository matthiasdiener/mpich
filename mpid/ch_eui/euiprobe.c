 ...
 */
int MPID_EUI_Iprobe( tag, source, context_id, found, status )
...
if (!*found) {
    ...
    MPID_EUI_check_incoming( MPID_NOTBLOCKING );
    DMPI_search_unexpected_queue( source, tag, context_id, 
				  found, 0, &dmpi_unexpected );
    }
if (*found) {
...
void MPID_EUI_Probe( tag, source, context_id, status )
...
while (1) {
    MPID_EUI_Iprobe( tag, source, context_id, &found, status );
    if (found) break;
    /* Wait for a message */
    MPID_EUI_check_incoming( MPID_BLOCKING );
    }
