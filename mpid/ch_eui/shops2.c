...
/*
    MPID_EUI_init_barrier returns a pointer to a MPID_Fastbar structure.
...
double MPID_EUI_Barrier_sum_double(bar,x)
MPID_Fastbar *bar;
double *x;
{
    register int oldphase;
...
double MPID_EUI_Barrier_max_double(bar,x)
MPID_Fastbar *bar;
double *x;
{
    register int oldphase;
...
double MPID_EUI_Barrier_min_double(bar,x)
MPID_Fastbar *bar;
double *x;
{
    register int oldphase;
...
char **argv;
{
    ...
    starttime = p4_ustimer();
    for (i = 0; i < 1000; i++)
      ysum = MPID_EUI_Barrier_sum_double(bar,&x);
    endtime = p4_ustimer();
