...
/* buffer.                                                                 */
/***************************************************************************/
int MPID_EUI_Copy_body_short( dmpi_recv_handle, pkt, pktbuf )
...
int MPID_EUI_Copy_body_sync_short( dmpi_recv_handle, pkt, from )
MPIR_RHANDLE *dmpi_recv_handle;
MPID_PKT_T   *pkt;
int          from;
{
...
/* 
    ...
    sender for the data...
 */
int MPID_EUI_Copy_body_long( dmpi_recv_handle, pkt, from )
MPIR_RHANDLE *dmpi_recv_handle;
MPID_PKT_T   *pkt;
int          from;
{
...
{__EUIFROM=-1;__EUITYPE=MPID_PT2PT2_TAG(from);mpc_brecv(mpid_recv_handle->start,msglen,&__EUIFROM,&__EUITYPE,&__EUILEN);};
DMPI_mark_recv_completed(dmpi_recv_handle);

return err;
}

int MPID_EUI_Copy_body_sync_long( dmpi_recv_handle, pkt, from )
MPIR_RHANDLE *dmpi_recv_handle;
MPID_PKT_T   *pkt;
int          from;
{
...
if (mpid_recv_handle_unex->send_id) {
    MPID_EUI_Ack_Request( mpid_recv_handle, mpid_recv_handle_unex->from,
...
/*
   ...
   processing.
 */
int MPID_EUI_post_recv( dmpi_recv_handle, is_available ) 
MPIR_RHANDLE *dmpi_recv_handle;
int          *is_available;
{
...
while (MPID_EUI_check_incoming( MPID_NOTBLOCKING ) != -1) ;
...
switch (pkt->head.mode) {
    ...
    /* Send back an OK to send, with a tag value and 
       a posted recv */
    MPID_EUI_Ack_Request( &dmpi_recv_handle->dev_rhandle, from, 
...
switch (pkt->head.mode) {
	...
	MPIDGETMEM(msglen);
	MPID_KEEP_STAT(n_long++;)
	{__EUIFROM=-1;__EUITYPE=MPID_PT2PT2_TAG(from);mpc_brecv(address,msglen,&__EUIFROM,&__EUITYPE,&__EUILEN);};
	break;
#endif
    }
...
if (DebugFlag) {
	...
	switch (MPID_PKT.head.mode) {
	    case MPID_PKT_SHORT_READY:
	    case MPID_PKT_SHORT:
	    err = MPID_EUI_Copy_body_short( dmpi_recv_handle, &MPID_PKT, 
	...
	case MPID_PKT_LONG_READY:
	case MPID_PKT_LONG:
	    err = MPID_EUI_Copy_body_long( dmpi_recv_handle, &MPID_PKT, from );
	    break;
	case MPID_PKT_LONG_SYNC:
	    err = MPID_EUI_Copy_body_sync_long( dmpi_recv_handle, &MPID_PKT, 
					       from );
	    break;
#endif
	case MPID_PKT_SHORT_SYNC:
	    err = MPID_EUI_Copy_body_sync_short( dmpi_recv_handle, &MPID_PKT, 
					        from );
	    break;
	    }
	}
    else {
	MPID_EUI_Copy_body_unex( dmpi_recv_handle, &MPID_PKT, from );
	}
    }
else {
    switch (MPID_PKT.head.mode) {
	...
	MPID_EUI_Do_Request( MPID_PKT.sendok_pkt.use_tag, from, 
...
if (!dmpi_recv_handle->completed && dmpi_recv_handle->dev_rhandle.rid) {
    MPID_EUI_Complete_Rndv( &dmpi_recv_handle->dev_rhandle );
...
#endif                  /* #DEBUG_END# */
	if (is_posted) {
	    err = MPID_EUI_Copy_body( dmpi_recv_handle, &MPID_PKT, from );
#ifdef MPID_USE_RNDV
...
/* Respond to a request to send a message when the message is found to
   be posted */
int MPID_EUI_Ack_Request( mpid_recv_handle, from, send_id )
MPID_RHANDLE *mpid_recv_handle;
int          from;
MPID_Aint    send_id;
{
...
{__EUIFROM=-1;__EUITYPE=tag;mpc_recv(mpid_recv_handle->start,mpid_recv_handle->bytes_as_contig,&__EUIFROM,&__EUITYPE,&(mpid_recv_handle->rid ));};
#endif
...
mpc_bsend(&pkt,sizeof(MPID_PKT_OK_TO_SEND_T),from,MPID_PT2PT_TAG);
return MPI_SUCCESS;
}
...
mp_wait(&(mpid_recv_handle->rid ),&__EUILEN);;
mpid_recv_handle->rid = 0;
}

/* Fullfill a request for a message */
int MPID_EUI_Do_Request( use_tag, from, send_id )
int          use_tag, from;
MPID_Aint    send_id;
{
...
mpc_send(dmpi_send_handle->dev_shandle.start,dmpi_send_handle->dev_shandle.bytes_as_contig,from,use_tag,&(mpid_send_handle->sid ));
return MPI_SUCCESS;
}
#endif
