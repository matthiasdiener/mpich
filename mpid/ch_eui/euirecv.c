...
/* 
  ...
  a MPID_EUI_check_incoming routine that looks for headers.  Note that
...
/* them.                                                                   */
/***************************************************************************/
static int MPID_EUI_Copy_body_short( dmpi_recv_handle, pkt, pktbuf )
...
msglen = pkt->head.len;
MPID_EUIK_MSGLEN(dmpi_recv_handle,msglen,err)
...
static int MPID_EUI_Copy_body_sync_short( dmpi_recv_handle, pkt, from )
MPIR_RHANDLE *dmpi_recv_handle;
MPID_PKT_T   *pkt;
int          from;
{
...
err = MPID_EUI_Copy_body_short( dmpi_recv_handle, pkt, 
...
/* 
    ...
    sender for the data...
 */
static int MPID_EUI_Copy_body_long( dmpi_recv_handle, pkt, from )
MPIR_RHANDLE *dmpi_recv_handle;
MPID_PKT_T   *pkt;
int          from;
{
...
int MPID_EUI_Cmpl_recv_nb( dmpi_recv_handle )
MPIR_RHANDLE *dmpi_recv_handle;
{
}

int MPID_EUI_Cmpl_recv_sync( dmpi_recv_handle )
MPIR_RHANDLE *dmpi_recv_handle;
{
/* ??? */
while (!MPID_Test_handle(dmpi_recv_handle)) {
    (void)MPID_EUI_check_incoming( MPID_BLOCKING );
    }
}

static int MPID_EUI_Copy_body_sync_long( dmpi_recv_handle, pkt, from )
MPIR_RHANDLE *dmpi_recv_handle;
MPID_PKT_T   *pkt;
int          from;
{
...
err = MPID_EUI_Copy_body_long( dmpi_recv_handle, pkt, from );
 ...
 */
int MPID_EUI_Process_unexpected( dmpi_recv_handle, dmpi_unexpected )
...
MPID_EUI_complete_recv( dmpi_unexpected );
 ...
 */
int MPID_EUI_post_recv( dmpi_recv_handle ) 
MPIR_RHANDLE *dmpi_recv_handle;
{
...
if (!dmpi_recv_handle->dev_rhandle.is_non_blocking) {
    return MPID_EUI_blocking_recv( dmpi_recv_handle );
    }
...
#ifdef MPID_USE_RNDV
    return MPID_EUI_Process_unexpected_rndv( dmpi_recv_handle, 
					    dmpi_unexpected );
#elif defined(MPID_USE_GET)
    return MPID_EUI_Process_unexpected_get( dmpi_recv_handle, 
					    dmpi_unexpected );
#else
    return MPID_EUI_Process_unexpected( dmpi_recv_handle, dmpi_unexpected );
#endif
    }
 ...
 */
int MPID_EUI_Copy_body( dmpi_recv_handle, pkt, from )
MPIR_RHANDLE *dmpi_recv_handle;
MPID_PKT_T   *pkt;
int          from;
{
...
switch (pkt->head.mode) {
    ...
    err = MPID_EUI_Copy_body_sync_short( dmpi_recv_handle, pkt, from );
    DMPI_mark_recv_completed(dmpi_recv_handle);
    break;
    ...
    MPID_EUI_Ack_Request( dmpi_recv_handle, from, pkt->request_pkt.send_id,
			 pkt->head.len );
    ...
    MPID_EUI_Do_get( dmpi_recv_handle, from, (MPID_PKT_GET_T *)pkt );
    ...
    case MPID_PKT_LONG:
#ifdef MPID_USE_RNDV
    err = MPID_EUI_Copy_body_long_rndv( dmpi_recv_handle, pkt, from );
#else
    err = MPID_EUI_Copy_body_long( dmpi_recv_handle, pkt, from );
#endif
    DMPI_mark_recv_completed(dmpi_recv_handle);
    ...
    err = MPID_EUI_Copy_body_sync_long( dmpi_recv_handle, pkt, from );
    DMPI_mark_recv_completed(dmpi_recv_handle);
    break;
#endif
    }
...
int MPID_EUI_Copy_body_unex( dmpi_recv_handle, pkt, from )
MPIR_RHANDLE *dmpi_recv_handle;
MPID_PKT_T   *pkt;
int          from;
{
...
switch (pkt->head.mode) {
	...
	MPID_EUI_Do_get_to_mem( address, from, (MPID_PKT_GET_T *)pkt );
...
/* dispatches them.  There is another such look in MPID_EUI_blocking_recv   */
 ...
 */    
int MPID_EUI_check_incoming( is_blocking )
...
/* Look for XDR bit */
	...
	switch (MPID_PKT_RECV_GET(pkt,head.mode)) {
	    case MPID_PKT_SHORT_READY:
	    case MPID_PKT_SHORT:
	    err = MPID_EUI_Copy_body_short( dmpi_recv_handle, 
	...
	case MPID_PKT_REQUEST_SEND_READY:
	    ...
	    MPID_EUI_Ack_Request( dmpi_recv_handle, from, 
	...
	case MPID_PKT_LONG:
#ifdef MPID_USE_RNDV
	    err = MPID_EUI_Copy_body_long_rndv( dmpi_recv_handle, 
					       MPID_PKT_RECV_ADDR(pkt), from );
#else
	    err = MPID_EUI_Copy_body_long( dmpi_recv_handle, 
					  MPID_PKT_RECV_ADDR(pkt), from );
#endif
	    break;
	case MPID_PKT_LONG_SYNC:
	    err = MPID_EUI_Copy_body_sync_long( dmpi_recv_handle, 
					       MPID_PKT_RECV_ADDR(pkt), 
					       from );
	    break;
#endif
	case MPID_PKT_SHORT_SYNC:
	    err = MPID_EUI_Copy_body_sync_short( dmpi_recv_handle, 
					        MPID_PKT_RECV_ADDR(pkt), 
					        from );
	    break;
#ifdef MPID_USE_GET
        case MPID_PKT_DO_GET:
	    MPID_EUI_Do_get( dmpi_recv_handle, from, 
	...
	MPID_EUI_Copy_body_unex( dmpi_recv_handle, MPID_PKT_RECV_ADDR(pkt), 
			        from );
	}
    }
else {
	...
	MPID_EUI_Do_Request( MPID_PKT_RECV_GET(pkt,sendok_pkt.recv_handle), 
...
#ifdef MPID_USE_GET
        case MPID_PKT_DONE_GET:
        MPID_EUI_Done_get( MPID_PKT_RECV_ADDR(pkt), from );
        break;
	case MPID_PKT_CONT_GET:
	MPID_EUI_Cont_get( MPID_PKT_RECV_ADDR(pkt), from );
	break;
#endif
 ...
 */
int MPID_EUI_complete_recv( dmpi_recv_handle ) 
MPIR_RHANDLE *dmpi_recv_handle;
{
...
switch (dmpi_recv_handle->completer) {
    ...
    MPID_EUI_Cmpl_recv_rndv( dmpi_recv_handle );
    break;
#endif
#ifndef MPID_USE_RNDV
    case MPID_CMPL_RECV_NB:
    MPID_EUI_Cmpl_recv_nb( dmpi_recv_handle );
    break;
#endif
#ifdef MPID_USE_GET    
...
int MPID_EUI_Test_recv_push( dmpi_recv_handle )
MPIR_RHANDLE *dmpi_recv_handle;
{
#ifdef MPID_USE_RNDV
if (dmpi_recv_handle->completer == MPID_CMPL_RECV_RNDV) {
    return MPID_EUI_Test_recv_rndv( dmpi_recv_handle );
    }
#endif
...
/*
   ...
   complete-recv).
 */
int MPID_EUI_blocking_recv( dmpi_recv_handle ) 
MPIR_RHANDLE *dmpi_recv_handle;
{
MPID_PKT_RECV_DECL(MPID_PKT_T,pkt);
...
#endif                  /* #DEBUG_END# */
	if (is_posted) {
	    err = MPID_EUI_Copy_body( dmpi_recv_handle, 
				     MPID_PKT_RECV_ADDR(pkt), from );
