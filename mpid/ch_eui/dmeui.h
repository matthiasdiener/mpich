...
#include <stdio.h>
extern int __NUMNODES,__MYPROCID,__ALLGRP,__EUILEN,__EUIFROM,__EUITYPE;static double __EUI_DBL;
/* #MEIKO DECLARATION# */
#else
/* 
 ...
 */
#define MPID_EUIK_MSGLEN(dmpi_recv_handle,msglen,err) \
...
#define MPID_PKT_PACK(pkt,size,dest) MPID_EUI_Pkt_pack(pkt,size,dest)
#define MPID_PKT_UNPACK(pkt,size,src) MPID_EUI_Pkt_unpack(pkt,size,src)
#else
