...
#define MPIDTRANSPORT "IBM EUI/EUIH"
...
#include "mpproto.h"
extern int __NUMNODES,__MYPROCID,__EUILEN,__EUIFROM,__EUITYPE;static double __EUI_DBL;
/* #MEIKO DECLARATION# */
#else
/* 
...
typedef struct {
    ...
    int           bytes_as_contig;
        /* Rest of data */
    struct { int rid, tag, from; } rid;              /* Id of non-blocking recv, if used.
...
#define MPID_Post_send_sync(ctx,dmpi_send_handle) \
    MPID_EUI_post_send_sync(dmpi_send_handle) 
...
#define MPID_Blocking_send(ctx, dmpi_send_handle) \
    MPID_EUI_Blocking_send(dmpi_send_handle)
#else
#define MPID_Blocking_send(ctx, dmpi_send_handle) \
{int _err;\
...
#define MPID_Test_send( ctx, dmpi_send_handle ) \
    ((dmpi_send_handle)->completer == 0 ? \
     1 : MPID_EUI_Test_send( dmpi_send_handle ))

#define MPID_Post_recv(ctx,dmpi_recv_handle ) \
    MPID_EUI_post_recv(dmpi_recv_handle ) 
...
#define MPID_Test_recv( ctx, dmpi_recv_handle ) \
    ((dmpi_recv_handle)->completer == 0 ? \
        1 : MPID_EUI_Test_recv_push( dmpi_recv_handle )) 
...
#define MPID_Test_request( ctx, request ) \
    ( (request)->chandle.handle_type == MPIR_SEND ? \
        MPID_EUI_Test_send(&(request)->shandle) : \
        MPID_EUI_Test_recv_push(&(request)->rhandle))
...
#define MPID_Version_name( ctx, name ) MPID_EUI_Version_name( name )
#ifndef MPID_WTIME
#define MPID_WTIME(ctx)         MPID_EUI_Wtime()
#endif
#define MPID_WTICK(ctx)         MPID_EUI_Wtick()
#define MPID_INIT(argc,argv) MPID_EUI_Init( argc, argv )
#define MPID_END(ctx)           MPID_EUI_End()

#define MPID_ABORT( ctx, errorcode ) MPID_EUI_Abort( errorcode );

#define MPID_CANCEL( ctx, r ) MPID_EUI_Cancel( r )

#define MPID_Myrank( ctx, rank ) MPID_EUI_Myrank( rank )
#define MPID_Mysize( ctx, size ) MPID_EUI_Mysize( size )
...
#define MPID_Comm_init(ctx,comm,newcomm) MPID_EUI_Comm_init(comm,newcomm)
#define MPID_Comm_free(ctx,comm) MPID_EUI_Comm_free(comm)
#define MPID_Barrier(ctx,comm) MPID_EUI_Barrier(comm)
...
#define MPID_Reduce_sum_int(ctx,send,recv,comm) \
      MPID_EUI_Reduce_sum_int(send,recv,comm)
#define MPID_Reduce_sum_double(ctx,send,recv,comm) \
      MPID_EUI_Reduce_sum_double(send,recv,comm)
...
#define MPID_Comm_init(ctx,comm,newcomm) MPID_EUI_Comm_msgrep( newcomm )
#else
 ...
 */
#ifdef MPID_HAS_HETERO
#define MPID_PKT_PACK(pkt,size,dest) MPID_EUI_Pkt_pack((MPID_PKT_T*)(pkt),size,dest)
#define MPID_PKT_UNPACK(pkt,size,src) MPID_EUI_Pkt_unpack((MPID_PKT_T*)(pkt),size,src)
#else
...
#define MPID_Dest_byte_order(dest) MPID_EUI_Dest_byte_order(dest)
#endif 
