...
if (MPID_IS_HETERO &&
    ...
    SYByteSwapInt( (int*)&pkt, sizeof(MPID_PKT_HEAD_T) / sizeof(int) );
    }
#endif                       /* #HETERO_END# */
...
#ifdef MPID_DEBUG_ALL   /* #DEBUG_START# */
    if (DebugFlag) {
	MPID_EUI_Print_pkt_data( "S Getting data from mpid->start", 
			        pkt.buffer, len );
	}
#endif                  /* #DEBUG_END# */
    }
...
#endif
mpc_bsend(&pkt,len + sizeof(MPID_PKT_HEAD_T),dest,MPID_PT2PT_TAG);
...
/* Long message */
int MPID_EUI_post_send_long( dmpi_send_handle, mpid_send_handle, len ) 
MPIR_SHANDLE *dmpi_send_handle;
MPID_SHANDLE *mpid_send_handle;
int len;
{
...
if (mpid_send_handle->is_non_blocking) 
    while (MPID_EUI_check_incoming( MPID_NOTBLOCKING ) != -1) ;
#endif
mpc_bsend(&pkt,sizeof(MPID_PKT_LONG_T),dest,MPID_PT2PT_TAG);
...
#ifndef PI_NO_NSEND
if (mpid_send_handle->is_non_blocking) {
    mpc_send(address,len,dest,MPID_PT2PT2_TAG(__MYPROCID),&(mpid_send_handle->sid ));
    }
else 
#endif
    {
    mpid_send_handle->sid = 0;
    mpc_bsend(address,len,dest,MPID_PT2PT2_TAG(__MYPROCID));
    DMPI_mark_send_completed( dmpi_send_handle );
    }
#endif
return MPI_SUCCESS;
...
if (dmpi_send_handle->msgrep == MPIR_MSGREP_XDR) 
    pkt.has_xdr = MPID_MODE_XDR;
#endif                         /* #HETERO_END# */
pkt.sync_id      = MPID_EUI_Get_Sync_Id( dmpi_send_handle, mpid_send_handle );
...
if (MPID_IS_HETERO &&
    ...
    /* Need to swap to receiver's order.  We ALWAYS reorder at the
       ...
       that matches the receiver's ordering without requiring a user-unpack.
       We may need to generalize this for XDR systems... */
    SYByteSwapInt( (int *)&pkt, sizeof(MPID_PKT_HEAD_T) / sizeof(int) + 1 );
    }
#endif                       /* #HETERO_END# */
...
/* In case the message is marked as non-blocking, indicate that we don't
   need to wait on it */
mpid_send_handle->sid = 0;
mpc_bsend(&pkt,len + (sizeof(MPID_PKT_SHORT_SYNC_T)-MPID_PKT_MAX_DATA_SIZE),dest,MPID_PT2PT_TAG);
...
/* Long message */
int MPID_EUI_post_send_sync_long( dmpi_send_handle, mpid_send_handle, len ) 
MPIR_SHANDLE *dmpi_send_handle;
MPID_SHANDLE *mpid_send_handle;
int len;
{
...
#endif                  /* #DEBUG_END# */
/* Send as packet only */
mpc_bsend(&pkt,sizeof(MPID_PKT_LONG_SYNC_T),dest,MPID_PT2PT_TAG);
...
if (actual_len > MPID_PKT_DATA_SIZE) 
    return MPID_EUI_post_send_long( dmpi_send_handle, mpid_send_handle, 
				   actual_len );
else
    return MPID_EUI_post_send_short( dmpi_send_handle, mpid_send_handle, 
				    actual_len );
}
...
if (actual_len > MPID_PKT_DATA_SIZE) 
    ...
    return MPID_EUI_post_send_long( dmpi_send_handle, mpid_send_handle, 
				   actual_len );
#else
    return MPID_EUI_post_send_sync_long( dmpi_send_handle, mpid_send_handle, 
				        actual_len );
#endif
else
    return MPID_EUI_post_send_sync_short( dmpi_send_handle, mpid_send_handle, 
					 actual_len );
}
...
if (mpid_send_handle->sid) {
    ...
    /* We do this to keep us from blocking in a wait in the event that
       ...
       wait. */
    while (!(mp_status(&(mpid_send_handle->sid)) > -1 ))
	(void) MPID_EUI_check_incoming( MPID_NOTBLOCKING );
#endif
    {int d; mp_wait(&(mpid_send_handle->sid ),&d);};
    mpid_send_handle->sid = 0;
    }
#endif
