...
int MPID_EUI_Cmpl_send_sync( dmpi_send_handle )
MPIR_SHANDLE *dmpi_send_handle;
{
MPID_SHANDLE *mpid_send_handle = &dmpi_send_handle->dev_shandle;
#ifndef PI_NO_NSEND
...
if (actual_len > MPID_PKT_DATA_SIZE) 
    ...
    rc = MPID_EUI_post_send_long_rndv( dmpi_send_handle, mpid_send_handle, 
				      actual_len );
#else
    rc = MPID_EUI_post_send_long( dmpi_send_handle, mpid_send_handle, 
				   actual_len );
#endif
else
...
if (actual_len > MPID_PKT_DATA_SIZE) 
    ...
    rc = MPID_EUI_post_send_sync_long_get( dmpi_send_handle, mpid_send_handle, 
					  actual_len );
#else
    rc = MPID_EUI_post_send_sync_long( dmpi_send_handle, mpid_send_handle, 
				      actual_len );
#endif
else
...
switch (dmpi_send_handle->completer) {
    ...
    case MPID_CMPL_SEND_GET:
	 MPID_EUI_Cmpl_send_get( dmpi_send_handle );
         break;
#endif
#ifndef PI_NO_NSEND
...
\tmode       = ", 
    ...
    case MPID_PKT_OK_TO_SEND:
    MPID_EUI_Rndv_print_pkt( fp, pkt );
    break;
#endif
#ifdef MPID_USE_GET
    case MPID_PKT_DO_GET:
