...
/* Send a short (single packet message) 
   ...
   macros expect dest) 
*/
int MPID_EUI_send_short( buf, len, tag, context_id, lrank_sender, dest,
		        msgrep ) 
void *buf;
...
int MPID_EUI_post_send_short( dmpi_send_handle, mpid_send_handle, len ) 
...
int MPID_EUI_post_send_long_eager( dmpi_send_handle, mpid_send_handle, len ) 
MPIR_SHANDLE *dmpi_send_handle;
MPID_SHANDLE *mpid_send_handle;
int len;
{
...
#ifndef PI_NO_NSEND           
MPID_EUI_Cmpl_send_nb( dmpi_send_handle )
MPIR_SHANDLE *dmpi_send_handle;
{
...
if (mpid_send_handle->sid)  {
    ...
    (void)MPID_EUI_check_incoming( MPID_NOTBLOCKING );
    MPID_EUI_isend_wait( dmpi_send_handle );
    }
...
int MPID_EUI_post_send_sync_short( dmpi_send_handle, mpid_send_handle, len ) 
MPIR_SHANDLE *dmpi_send_handle;
MPID_SHANDLE *mpid_send_handle;
int len;
{
...
MPID_PKT_SEND_SET(pkt,sync_id,
		  MPID_EUI_Get_Sync_Id( dmpi_send_handle, mpid_send_handle ));
...
#ifdef MPID_USE_GET               
#else
int MPID_EUI_post_send_sync_long_eager( dmpi_send_handle, mpid_send_handle, 
				       len ) 
MPIR_SHANDLE *dmpi_send_handle;
MPID_SHANDLE *mpid_send_handle;
int len;
...
void MPID_EUI_Cmpl_send_sync( dmpi_send_handle )
MPIR_SHANDLE *dmpi_send_handle;
{
MPID_SHANDLE *mpid_send_handle = &dmpi_send_handle->dev_shandle;
...
while (!MPID_Test_handle(dmpi_send_handle)) {
    /* This waits for the completion of a synchronous send, since at
       this point, we've finished waiting for the mpc_send(,,,,&()) to complete,
...
int len;
{
return MPID_EUI_post_send_long_rndv( dmpi_send_handle, mpid_send_handle, len );
}
 ...
 ***************************************************************************/
int MPID_EUI_Post_send_local( dmpi_send_handle, mpid_send_handle, len )
MPIR_SHANDLE *dmpi_send_handle;
MPID_SHANDLE *mpid_send_handle;
int           len;
{
 ...
 */
int MPID_EUI_post_send( dmpi_send_handle ) 
MPIR_SHANDLE *dmpi_send_handle;
{
MPID_SHANDLE *mpid_send_handle;
int         actual_len, rc;
...
if (dest == MPID_MyWorldRank) {
    return MPID_EUI_Post_send_local( dmpi_send_handle, mpid_send_handle, 
				    actual_len );
    }
 }
#endif
...
#ifdef MPID_USE_GET
    rc = MPID_EUI_post_send_long_get( dmpi_send_handle, mpid_send_handle, 
				   actual_len );
#elif defined(MPID_USE_RNDV)              
    rc = MPID_EUI_post_send_long_rndv( dmpi_send_handle, mpid_send_handle, 
				      actual_len );
#else
    rc = MPID_EUI_post_send_long_eager( dmpi_send_handle, mpid_send_handle, 
				       actual_len );
                                          
#endif
else
    rc = MPID_EUI_post_send_short( dmpi_send_handle, mpid_send_handle, 
				    actual_len );
...
int MPID_EUI_post_send_sync( dmpi_send_handle ) 
MPIR_SHANDLE *dmpi_send_handle;
{
MPID_SHANDLE *mpid_send_handle;
int         actual_len, rc;
...
if (actual_len > MPID_PKT_DATA_SIZE) 
    ...
    rc = MPID_EUI_post_send_sync_long_get( dmpi_send_handle, mpid_send_handle, 
					  actual_len );
#elif defined(MPID_USE_RNDV)      
    rc = MPID_EUI_post_send_long_rndv( dmpi_send_handle, mpid_send_handle, 
				      actual_len );
#else
    rc = MPID_EUI_post_send_sync_long_eager( dmpi_send_handle, 
					    mpid_send_handle, actual_len );
                                  
#endif
else
    rc = MPID_EUI_post_send_sync_short( dmpi_send_handle, mpid_send_handle, 
					 actual_len );
 ...
 */
int MPID_EUI_Contig_blocking_send( buf, len, tag, context_id, lrank_sender, 
				  grank_dest, msgrep )
void *buf;
...
if (len <= MPID_PKT_DATA_SIZE) 
    rc = MPID_EUI_send_short( buf, len, tag, context_id, lrank_sender, 
			     grank_dest, msgrep );
else 
#ifdef MPID_USE_GET
    rc = MPID_EUI_send_long_get( buf, len, tag, context_id, lrank_sender, 
			        grank_dest, msgrep );
#ifdef FOO
#elif defined(MPID_USE_RNDV)              
    rc = MPID_EUI_send_long_rndv( buf, len, tag, context_id, lrank_sender, 
			         grank_dest, msgrep);
#else
    rc = MPID_EUI_send_long_eager( buf, len, tag, context_id, lrank_sender, 
				  grank_dest, msgrep);
...
int MPID_EUI_Blocking_send( dmpi_send_handle )
MPIR_SHANDLE *dmpi_send_handle;
{
int err = MPI_SUCCESS;
...
err = MPID_EUI_post_send( dmpi_send_handle );
if (!err) MPID_EUI_complete_send( dmpi_send_handle );
 ...
 */
int MPID_EUI_isend_wait( dmpi_send_handle )
MPIR_SHANDLE *dmpi_send_handle;
{
MPID_SHANDLE *mpid_send_handle;
 ...
 */
int MPID_EUI_complete_send( dmpi_send_handle ) 
MPIR_SHANDLE *dmpi_send_handle;
{
int err = MPI_SUCCESS;
...
switch (dmpi_send_handle->completer) {
    ...
    case MPID_CMPL_SEND_RNDV:
	 MPID_EUI_Cmpl_send_rndv( dmpi_send_handle );
         break;
#endif                                 
#ifndef PI_NO_NSEND                    
 ...
 */
int MPID_EUI_Test_send( dmpi_send_handle )
MPIR_SHANDLE *dmpi_send_handle;
{
#ifdef MPID_USE_RNDV                           
MPID_EUI_Test_send_rndv( dmpi_send_handle );
#endif                                         
#ifndef PI_NO_NSEND
