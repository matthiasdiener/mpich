...
/* buffer.                                                                 */
/***************************************************************************/
int MPID_MEIKO_Copy_body_short( dmpi_recv_handle, pkt, pktbuf )
...
int MPID_MEIKO_Copy_body_sync_short( dmpi_recv_handle, pkt, from )
MPIR_RHANDLE *dmpi_recv_handle;
MPID_PKT_T   *pkt;
int          from;
{
...
/* 
    ...
    sender for the data...
 */
int MPID_MEIKO_Copy_body_long( dmpi_recv_handle, pkt, from )
MPIR_RHANDLE *dmpi_recv_handle;
MPID_PKT_T   *pkt;
int          from;
{
...
int MPID_MEIKO_Copy_body_sync_long( dmpi_recv_handle, pkt, from )
MPIR_RHANDLE *dmpi_recv_handle;
MPID_PKT_T   *pkt;
int          from;
{
...
if (mpid_recv_handle_unex->send_id) {
    MPID_MEIKO_Ack_Request( mpid_recv_handle, mpid_recv_handle_unex->from,
...
/*
   ...
   processing.
 */
int MPID_MEIKO_post_recv( dmpi_recv_handle, is_available ) 
MPIR_RHANDLE *dmpi_recv_handle;
int          *is_available;
{
...
switch (pkt->head.mode) {
    ...
    /* Send back an OK to send, with a tag value and 
       a posted recv */
    MPID_MEIKO_Ack_Request( &dmpi_recv_handle->dev_rhandle, from, 
...
if (DebugFlag) {
	...
	switch (MPID_PKT.head.mode) {
	    case MPID_PKT_SHORT_READY:
	    case MPID_PKT_SHORT:
	    err = MPID_MEIKO_Copy_body_short( dmpi_recv_handle, &MPID_PKT, 
	...
	case MPID_PKT_LONG_READY:
	case MPID_PKT_LONG:
	    err = MPID_MEIKO_Copy_body_long( dmpi_recv_handle, &MPID_PKT, from );
	    break;
	case MPID_PKT_LONG_SYNC:
	    err = MPID_MEIKO_Copy_body_sync_long( dmpi_recv_handle, &MPID_PKT, 
					       from );
	    break;
#endif
	case MPID_PKT_SHORT_SYNC:
	    err = MPID_MEIKO_Copy_body_sync_short( dmpi_recv_handle, &MPID_PKT, 
					        from );
	    break;
	    }
	}
    else {
	MPID_MEIKO_Copy_body_unex( dmpi_recv_handle, &MPID_PKT, from );
	}
    }
else {
    switch (MPID_PKT.head.mode) {
	...
	MPID_MEIKO_Do_Request( MPID_PKT.sendok_pkt.use_tag, from, 
...
#endif                  /* #DEBUG_END# */
	if (is_posted) {
	    err = MPID_MEIKO_Copy_body( dmpi_recv_handle, &MPID_PKT, from );
#ifdef MPID_USE_RNDV
...
/* Respond to a request to send a message when the message is found to
   be posted */
int MPID_MEIKO_Ack_Request( mpid_recv_handle, from, send_id )
MPID_RHANDLE *mpid_recv_handle;
int          from;
MPID_Aint    send_id;
{
...
int MPID_MEIKO_Do_Request( use_tag, from, send_id )
int          use_tag, from;
MPID_Aint    send_id;
{
