...
#ifdef MPID_DEBUG_ALL   /* #DEBUG_START# */
    if (DebugFlag) {
	MPID_MEIKO_Print_pkt_data( "S Getting data from mpid->start", 
			        pkt.buffer, len );
	}
#endif                  /* #DEBUG_END# */
    }
...
/* Long message */
int MPID_MEIKO_post_send_long( dmpi_send_handle, mpid_send_handle, len ) 
MPIR_SHANDLE *dmpi_send_handle;
MPID_SHANDLE *mpid_send_handle;
int len;
{
...
pkt.tag		 = dmpi_send_handle->tag;
pkt.len          = len;
pkt.sync_id      = MPID_MEIKO_Get_Sync_Id( dmpi_send_handle, mpid_send_handle );
...
/* Long message */
int MPID_MEIKO_post_send_sync_long( dmpi_send_handle, mpid_send_handle, len ) 
MPIR_SHANDLE *dmpi_send_handle;
MPID_SHANDLE *mpid_send_handle;
int len;
{
...
if (actual_len > MPID_PKT_DATA_SIZE) 
    return MPID_MEIKO_post_send_long( dmpi_send_handle, mpid_send_handle, 
				   actual_len );
else
    return MPID_MEIKO_post_send_short( dmpi_send_handle, mpid_send_handle, 
				    actual_len );
}
...
if (actual_len > MPID_PKT_DATA_SIZE) 
    ...
    return MPID_MEIKO_post_send_long( dmpi_send_handle, mpid_send_handle, 
				   actual_len );
#else
    return MPID_MEIKO_post_send_sync_long( dmpi_send_handle, mpid_send_handle, 
				        actual_len );
#endif
else
    return MPID_MEIKO_post_send_sync_short( dmpi_send_handle, mpid_send_handle, 
					 actual_len );
}
