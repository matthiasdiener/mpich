/* 
 *   $Id: iread_sh.c,v 1.4 1999/08/27 20:53:09 thakur Exp $    
 *
                          ...
                          MPI_Datatype datatype, MPIO_Request *request)
{
    ...
    if (count < 0) {
	printf("MPI_File_iread_shared: Invalid count argument\n");
	MPI_Abort(MPI_COMM_WORLD, 1);
    }

    if (datatype == MPI_DATATYPE_NULL) {
        printf("MPI_File_iread_shared: Invalid datatype\n");
        MPI_Abort(MPI_COMM_WORLD, 1);
    }
    ...
    if ((count*datatype_size) % fh->etype_size != 0) {
        printf("MPI_File_iread_shared: Only an integral number of etypes can be accessed\n");
        MPI_Abort(MPI_COMM_WORLD, 1);
    }

    if ((fh->file_system == ADIO_PIOFS) || (fh->file_system == ADIO_PVFS)) {
	printf("MPI_File_iread_shared: Shared file pointer not supported on PIOFS and PVFS\n");
	MPI_Abort(MPI_COMM_WORLD, 1);
    }
	...
	printf("MPI_File_iread_shared: Could not access shared file pointer!\n");
	MPI_Abort(MPI_COMM_WORLD, 1);
    }
