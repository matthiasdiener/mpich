/* 
 *   $Id: read_atf.c,v 1.6 1999/08/27 20:53:33 thakur Exp $    
 *
...
void mpi_file_read_at_(MPI_Fint *fh,MPI_Offset *offset,void *buf,
      int *count,MPI_Fint *datatype,MPI_Status *status, int *__ierr )
{
    MPI_File fh_c;
    ...
    *__ierr = MPI_File_read_at(fh_c,*offset,buf,*count,datatype_c,status);
}
#else
void mpi_file_read_at_(MPI_Fint *fh,MPI_Offset *offset,void *buf,
      int *count,MPI_Datatype *datatype,MPI_Status *status, int *__ierr )
{
    MPI_File fh_c;
    
    fh_c = MPI_File_f2c(*fh);
    *__ierr = MPI_File_read_at(fh_c,*offset,buf,*count,*datatype,status);
}
#endif
