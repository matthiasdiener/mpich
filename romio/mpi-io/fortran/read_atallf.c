/* 
 *   $Id: read_atallf.c,v 1.6 1999/08/27 20:53:32 thakur Exp $    
 *
                         ...
                         MPI_Status *status, int *__ierr )
{
    ...
    *__ierr = MPI_File_read_at_all(fh_c,*offset,buf,*count,datatype_c,status);
}
#else
void mpi_file_read_at_all_(MPI_Fint *fh,MPI_Offset *offset,void *buf,
                         ...
                         MPI_Status *status, int *__ierr )
{
    ...
    fh_c = MPI_File_f2c(*fh);
    *__ierr = MPI_File_read_at_all(fh_c,*offset,buf,*count,*datatype,status);
}
#endif
