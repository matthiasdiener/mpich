/* 
 *   $Id: wr_atalle.c,v 1.4 1999/08/27 20:53:18 thakur Exp $    
 *
...
int MPI_File_write_at_all_end(MPI_File fh, void *buf, MPI_Status *status)
{
	...
	printf("MPI_File_write_at_all_end: Invalid file handle\n");
	MPI_Abort(MPI_COMM_WORLD, 1);
    }

    if (!(fh->split_coll_count)) {
        printf("MPI_File_write_at_all_end: Does not match a previous MPI_File_write_at_all_begin\n");
        MPI_Abort(MPI_COMM_WORLD, 1);
    }
