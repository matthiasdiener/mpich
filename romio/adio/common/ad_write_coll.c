/* 
 *   $Id: ad_write_coll.c,v 1.2 1998/06/02 18:56:49 thakur Exp $    
 *
...
static void ADIOI_Fill_send_buffer(ADIO_File fd, void *buf, ADIOI_Flatlist_node                           *flat_buf, char **send_buf, ADIO_Offset 
                           *offset_list, int *len_list, int *send_size, 
                           ...
                           int nprocs_for_coll, int myrank, 
...
/* PFS file pointer modes are not relevant here. */
	...
	printf("ADIOI_GEN_WriteStridedColl: only M_ASYNC and M_UNIX iomodes are valid\n");
	...
	fd->fp_ind = orig_fp;
        ...
        if (buftype_is_contig && filetype_is_contig) {
            ...
            if (file_ptr_type == ADIO_EXPLICIT_OFFSET) {
                ...
                ADIO_WriteContig(fd, buf, bufsize, ADIO_EXPLICIT_OFFSET,
                       off, status, error_code);
            }
            else ADIO_WriteContig(fd, buf, bufsize, ADIO_INDIVIDUAL,
                       0, status, error_code);
        }
...
/* exchange data and write in sizes of no more than coll_bufsize. */
    ADIOI_Exch_and_write(fd, buf, datatype, nprocs, nprocs_for_coll, myrank,
...
/* Send data to appropriate processes and write in sizes of no more
   ...
   at least another 8Mbytes of temp space is unacceptable. */
    ...
    ADIO_Offset st_loc=0, end_loc=0, off, done, req_off;
    char *write_buf;
    int *curr_offlen_ptr, *count, *send_size, req_len, *recv_size;
    int *partial_recv, *sent_to_proc, *start_pos, err=-1, flag;
    int *send_buf_idx, *curr_to_proc, *done_to_proc, err_flag=0;
    MPI_Status status;
...
/* ntimes=ceiling_div(end_loc - st_loc + 1, coll_bufsize)*/
    ...
    sent_to_proc = (int *) ADIOI_Calloc(nprocs_for_coll, sizeof(int));
    /* amount of data sent to each proc so far. Used in
       ADIOI_Fill_send_buffer. initialized to 0 here. */

    send_buf_idx = (int *) ADIOI_Malloc(nprocs_for_coll*sizeof(int));
    curr_to_proc = (int *) ADIOI_Malloc(nprocs_for_coll*sizeof(int));
    done_to_proc = (int *) ADIOI_Malloc(nprocs_for_coll*sizeof(int));
...
/* I need to check if there are any outstanding nonblocking writes to
   ...
   completes all pending nonblocking operations before completing. */
    ...
    for (m=0; m<ntimes; m++) {
	  ...
	  simplifies implementation a bit compared to reads. */
          ...
          /* off = start offset in the file for the data to be written in 
                   this iteration 
             size = size of data written corresponding to off
			...
			if (off+size-req_off < req_len) {
			    ...
			    if ((j+1 < others_req[i].count) && 
                                 (others_req[i].offsets[j+1] < 
                                     off+size)) { 
				printf("Error: the filetype specifies an overlapping write\n");
				MPI_Abort(MPI_COMM_WORLD, 1);
			    }
			    break;
			}
	...
	ADIOI_W_Exchange_data(fd, buf, write_buf, flat_buf, offset_list, 
                            ...
                            sent_to_proc, nprocs, nprocs_for_coll, myrank, 
	...
	if (flag) {
	    ADIO_WriteContig(fd, write_buf, size, ADIO_EXPLICIT_OFFSET, 
                        off, &status, &err);
	    if (err != MPI_SUCCESS) err_flag = 1;
	}
...
/* check if there are any holes */
    *hole = 0;
    ...
    if (nprocs_recv) {
	if (*hole) {
	    ADIO_ReadContig(fd, write_buf, size, ADIO_EXPLICIT_OFFSET, off,
			    &status, &err);
	    if (err != MPI_SUCCESS) {
		printf("ADIOI_GEN_WriteStridedColl: ROMIO tries to optimize this access by doing a read-modify-write, but is unable to read the file. Please give the file read permission and open it with MPI_MODE_RDWR.\n");
		MPI_Abort(MPI_COMM_WORLD, 1);
	    } 
	}
    }
    ...
    nprocs_send = 0;
    for (i=0; i<nprocs_for_coll; i++) if (send_size[i]) nprocs_send++;
...
/* post sends. if buftype_is_contig, data can be directly sent from
   user buf at location given by buf_idx. else use send_buf. */
	...
	send_buf = (char **) ADIOI_Malloc(nprocs_for_coll*sizeof(char*));
	for (i=0; i<nprocs_for_coll; i++) 
	    if (send_size[i]) 
	...
	ADIOI_Fill_send_buffer(fd, buf, flat_buf, send_buf,
			   ...
			   requests+nprocs_recv,
                           sent_to_proc, nprocs_for_coll, myrank, 
                           contig_access_count,
                           min_st_offset, fd_size, fd_start, fd_end, 
