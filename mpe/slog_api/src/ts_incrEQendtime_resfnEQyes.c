...
int main( int argc, char **argv )
{
    ...
    */
    ...
    /*  SLOG_SetFrameReservedSpace( slog, SLOG_typesz[ min_IntvlRec ] );  */
    /*  Here SLOG_Irec_ReserveSpace() will be used, so set 0 reserve space  */
    ...
    */ 
    ...
    SLOG_SetPreviewName( slog, "/dev/null" );
    ...
    */
    ...
    for ( Nassocs = 0; Nassocs <= MaxAssocs; Nassocs++ ) {
        ...
        if ( ierr != SLOG_SUCCESS ) {
            fprintf( errfile, __FILE__":Main() - "
                              "SLOG_RDEF_AddRecDef( Variable ) fails!\n" ); 
            fflush( errfile );
            exit( 1 );
        }
    }
    ...
    for ( idx_type   = FixIrec_MinIdx;
        ...
        for ( Nassocs = 0; Nassocs <= MaxAssocs; Nassocs++ ) {
            for ( Nargs = 0; Nargs <= MaxArgs; Nargs++ ) {
                ...
                if ( ierr != SLOG_SUCCESS ) {
                    fprintf( errfile, __FILE__":Main() - "
                                      "SLOG_RDEF_AddRecDef( Fixed ) fails!\n" );
                    fflush( errfile );
                    exit( 1 );
                }
            }
...
*/
    ...
    while ( ii < MaxIrec ) {
        ...
        */
        ...
        case 1 :
            irec_Nassocs  = 0;
            irec_Nargs    = idx2state % ( MaxIdx4VarIrec + 1 );
            break;
        case 2 :
            irec_Nassocs  = idx2state % ( MaxIdx4VarIrec + 1 );
            irec_Nargs    = 0;
            break;
        case 3 :
            irec_Nassocs  = idx2state % ( MaxIdx4VarIrec + 1 ); 
        ...
        */
        ...
        if (    idx2state % FixIrec_MaxIdx == 0
            ...
            irec_starttime   = prev_irec->starttime;
            irec_duration    = irec_endtime - irec_starttime;
            irec_orig_node   = (prev_irec->origID).node;
            irec_orig_cpu    = (prev_irec->origID).cpu;
            irec_orig_thread = (prev_irec->origID).thread;
    ...
    for ( ii = 0; ii < 2*MaxIrec; ii++ ) {
        ...
        */
        if ( events[ii].IsStartEvent == SLOG_TRUE ) {
            SLOG_Irec_ReserveSpace( slog, irec->rectype, irec->intvltype,
