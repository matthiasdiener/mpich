...
*/
  ...
  void changeZoomFactor (double z) {
    if (z <= 0) {new ErrorDialog (null, "Value must be > 0"); return;}
    zF = z;
  }
  //---------------------------------------------------------------------------------
  ...
  void changeToTime (double end) {
    if (end <= begTime) {
      new ErrorDialog (null, "value must be > " + Double.toString (begTime));
      return;
    }
  ...
  void changeFromTime (double start) {
    if (start >= endTime || start < 0) {
      new ErrorDialog (null, "value must be >= 0 and < "
   ...
   * CAUTION: setupNestedStates will hog up some of CPU's time but, it 
   * has to be performed to keep nesting upto date
   */
  void Refresh () {
    waitCursor ();
